...
function NumberStepper({ value, onChange, min = 0, max = 999, step = 1, label, hint, tarief, eenheid }: {
  ...
  return (
    <div className="rounded-xl p-3 mb-2" style={{ background: "var(--bg-surface)", border: "1px solid var(--planning-border-soft)" }}>
      <div className="flex items-center justify-between">
        <div className="flex-1 min-w-0">
          <p className="text-sm font-medium" style={{ color: "var(--text-primary)" }}>
            {label}
            {tarief != null && <span className="ml-1 text-[11px] font-normal" style={{ color: "var(--text-muted)", fontFamily: "DM Mono, monospace" }}>({euro(tarief)}/{eenheid || "st"})</span>}
          </p>
          {hint && <p className="text-[11px] mt-0.5" style={{ color: "var(--text-muted)" }}>{hint}</p>}
        </div>
        <div className="flex items-center gap-0 shrink-0 ml-3">
          <button onClick={() => onChange(Math.max(min, value - step))} className="w-8 h-8 rounded-l-lg flex items-center justify-center" style={{ background: "var(--app-navy)", border: "1px solid var(--planning-border-soft)" }}>
            <Minus className="h-3.5 w-3.5" style={{ color: "var(--text-muted)" }} />
          </button>
          <div className="w-12 h-8 flex items-center justify-center text-sm font-bold" style={{ background: "var(--app-navy)", borderTop: "1px solid var(--planning-border-soft)", borderBottom: "1px solid var(--planning-border-soft)", color: "var(--text-primary)", fontFamily: "DM Mono, monospace" }}>
            {value}
          </div>
          <button onClick={() => onChange(Math.min(max, value + step))} className="w-8 h-8 rounded-r-lg flex items-center justify-center" style={{ background: "var(--app-navy)", border: "1px solid var(--planning-border-soft)" }}>
...
function OptionButton({ selected, onClick, children, color }: { selected: boolean; onClick: () => void; children: React.ReactNode; color?: string }) {
  return (
    <button onClick={onClick} className="w-full text-left rounded-xl p-3 transition-all" style={{
      background: selected ? (color === "warn" ? "var(--warn-light)" : "var(--accent-light)") : "var(--bg-surface)",
      border: `1.5px solid ${selected ? (color === "warn" ? "var(--warn-border)" : "var(--accent-border)") : "var(--planning-border-soft)"}`,
...
function CheckRow({ checked, onChange, label, hint, tarief, eenheid }: {
  ...
  return (
    <button onClick={() => onChange(!checked)} className="w-full flex items-center gap-3 rounded-xl p-3 mb-1.5 text-left transition-all" style={{
      ...
      <input type="checkbox" checked={checked} readOnly className="w-[18px] h-[18px] shrink-0" style={{ accentColor: "var(--accent)" }} />
      <div className="flex-1 min-w-0">
        <p className="text-sm font-medium" style={{ color: "var(--text-primary)" }}>
          {label}
          {tarief != null && <span className="ml-1 text-[11px] font-normal" style={{ color: "var(--text-muted)", fontFamily: "DM Mono, monospace" }}>({euro(tarief)}/{eenheid || "st"})</span>}
...
function ToggleRow({ checked, onChange, label, hint }: { checked: boolean; onChange: (v: boolean) => void; label: string; hint?: string }) {
  return (
    <div className="flex items-center justify-between rounded-xl p-3 mb-1.5" style={{ background: "var(--bg-surface)", border: "1px solid var(--planning-border-soft)" }}>
      <div>
        <p className="text-sm font-medium" style={{ color: "var(--text-primary)" }}>{label}</p>
        {hint && <p className="text-[11px] mt-0.5" style={{ color: "var(--text-muted)" }}>{hint}</p>}
      </div>
      <button onClick={() => onChange(!checked)} className="w-11 h-6 rounded-full relative transition-colors shrink-0" style={{ background: checked ? "var(--accent)" : "var(--bg-surface-2)" }}>
  ...
  const renderStep = () => {
    switch (step) {
      ...
      case 0:
        return (
          <div className="space-y-3">
            <h3 className="text-base font-bold" style={{ color: "var(--text-primary)" }}>RMU installatie</h3>
            <div className="grid grid-cols-2 gap-2">
              <OptionButton selected={answers.rmu_vervangen} onClick={() => set({ rmu_vervangen: true })}>
                <div className="flex items-center gap-2"><Zap className="h-4 w-4" style={{ color: "var(--accent)" }} /><span className="text-sm font-semibold">RMU vervangen/plaatsen</span></div>
              </OptionButton>
              <OptionButton selected={!answers.rmu_vervangen} onClick={() => set({ rmu_vervangen: false, rmu_merk: null, rmu_configuratie_id: null, rmu_velden: 3 })}>
                <div className="flex items-center gap-2"><Minus className="h-4 w-4" style={{ color: "var(--text-muted)" }} /><span className="text-sm">Geen RMU</span></div>
              </OptionButton>
            </div>

            {answers.rmu_vervangen && (
              <div className="space-y-3 animate-fade-in">
                <p className="text-xs font-semibold" style={{ color: "var(--text-muted)" }}>Welk merk?</p>
                ...
                {answers.rmu_merk && (
                  <div className="space-y-2 animate-fade-in">
                    <p className="text-xs font-semibold" style={{ color: "var(--text-muted)" }}>Configuratie</p>
                    <div className="flex flex-wrap gap-2">
                      {merkConfigs.map(c => (
                        <button key={c.id} onClick={() => { set({ rmu_configuratie_id: c.id, rmu_velden: c.velden }); }} className="px-3 py-2 rounded-lg text-xs font-medium transition-all" style={{
                          ...
                          border: `1.5px solid ${answers.rmu_configuratie_id === c.id ? "var(--accent-border)" : "var(--planning-border-soft)"}`,
                          color: "var(--text-primary)",
                        }}>
                          <span className="font-bold">{c.code}</span> · {c.velden}-velds
                        </button>
                      ))}
                      <button onClick={() => setShowNewRmu(true)} className="px-3 py-2 rounded-lg text-xs" style={{ border: "1px dashed var(--planning-border-soft)", color: "var(--text-muted)" }}>+ Niet in lijst</button>
                    </div>

                    {showNewRmu && (
                      <div className="flex gap-2 items-end p-3 rounded-xl" style={{ background: "var(--bg-surface)", border: "1px solid var(--planning-border-soft)" }}>
                        <div className="flex-1">
                          <label className="text-[10px] block mb-1" style={{ color: "var(--text-muted)" }}>Code</label>
                    ...
                    {selectedRmu && (
                      <div className="rounded-xl p-3 animate-fade-in" style={{ background: "var(--accent-light)", border: "1px solid var(--accent-border)" }}>
                        <p className="text-xs font-semibold" style={{ color: "var(--accent)" }}>Op basis van {selectedRmu.merk} {selectedRmu.code} ({selectedRmu.velden}-velds):</p>
                        <p className="text-xs mt-1" style={{ color: "var(--text-primary)" }}>✓ R320010 Basis MS-installatie (1×)</p>
                        {selectedRmu.velden > 3 && <p className="text-xs" style={{ color: "var(--text-primary)" }}>✓ R320020 Extra MS-veld ({selectedRmu.velden - 3}×)</p>}
                        <p className="text-xs mt-1" style={{ color: "var(--text-muted)" }}>Kabelvelden: {selectedRmu.velden - 1} → suggestie {selectedRmu.velden - 1}× moffen + {selectedRmu.velden - 1}× eindsluitingen</p>
                      </div>
                    )}
                  </div>
                )}
      ...
      case 1:
        return (
          <div className="space-y-3">
            <h3 className="text-base font-bold" style={{ color: "var(--text-primary)" }}>MS extra werkzaamheden</h3>
            ...
            {!answers.ims_ombouw && !answers.trafokabel && <p className="text-xs italic" style={{ color: "var(--text-muted)" }}>Geen extra MS werkzaamheden</p>}
          </div>
        );
      ...
      case 2:
        return (
          <div className="space-y-3">
            <h3 className="text-base font-bold" style={{ color: "var(--text-primary)" }}>Transformator</h3>
                ...
                <OptionButton key={o.val} selected={answers.trafo_situatie === o.val} onClick={() => set({ trafo_situatie: o.val })}>
                  <div className="flex items-center gap-2">
                    ...
                    <div>
                      ...
                      {o.code && <p className="text-[11px]" style={{ color: "var(--text-muted)", fontFamily: "DM Mono, monospace" }}>{o.code} — {euro(getSpec(o.code)?.tarief || 0)}</p>}
                    </div>
                  </div>
                </OptionButton>
              ))}
      ...
      case 3:
        return (
          <div className="space-y-3">
            <h3 className="text-base font-bold" style={{ color: "var(--text-primary)" }}>LS-rek installatie</h3>
            <div className="space-y-2">
              {([
      ...
      case 4: {
        ...
        return (
          <div className="space-y-3">
            <div className="flex items-center justify-between">
              <h3 className="text-base font-bold" style={{ color: "var(--text-primary)" }}>Bouwkundige werkzaamheden</h3>
              {allZero && <button onClick={() => setStep(s => s + 1)} className="text-xs px-3 py-1.5 rounded-lg" style={{ border: "1px solid var(--planning-border-soft)", color: "var(--text-muted)" }}>Overslaan →</button>}
            </div>
            {allZero && <p className="text-xs italic" style={{ color: "var(--text-muted)" }}>Vul in als van toepassing, of sla over</p>}
      ...
      case 5:
        return (
          <div className="space-y-3">
            <h3 className="text-base font-bold" style={{ color: "var(--text-primary)" }}>MS moffen & eindsluitingen</h3>
            {answers.rmu_vervangen && selectedRmu && (
              <div className="rounded-xl p-3" style={{ background: "rgba(110,155,255,0.1)", border: "1px solid rgba(110,155,255,0.3)" }}>
                <p className="text-xs font-semibold" style={{ color: "var(--info)" }}>Suggestie op basis van {selectedRmu.merk} {selectedRmu.code} ({selectedRmu.velden - 1} kabelvelden):</p>
                <p className="text-xs mt-1" style={{ color: "var(--text-primary)" }}>{suggestie.moffen}× MS moffen + {suggestie.eindsluitingen}× eindsluitingen</p>
                <div className="flex gap-2 mt-2">
                  <button onClick={overneemSuggestie} className="px-3 py-1.5 rounded-lg text-xs font-bold text-white" style={{ background: "var(--accent)" }}>Overnemen</button>
                  <button className="px-3 py-1.5 rounded-lg text-xs" style={{ border: "1px solid var(--planning-border-soft)", color: "var(--text-muted)" }}>Zelf invullen</button>
                </div>
              </div>
            )}
            {isProvisiorium && (
              <div className="flex gap-2 p-3 rounded-r-xl" style={{ background: "var(--warn-light)", borderLeft: "3px solid var(--warn-text)" }}>
                <AlertTriangle className="h-4 w-4 shrink-0 mt-0.5" style={{ color: "var(--warn-text)" }} />
                <p className="text-xs font-medium" style={{ color: "var(--warn-text)" }}>Provisorium — vul eindsluitingen handmatig in. Niet automatisch afgeleid.</p>
              </div>
            )}
      ...
      case 6: {
        ...
        return (
          <div className="space-y-3">
            <div className="flex items-center justify-between">
              <h3 className="text-base font-bold" style={{ color: "var(--text-primary)" }}>LS moffen & eindsluitingen</h3>
              {allZero && <button onClick={() => setStep(s => s + 1)} className="text-xs px-3 py-1.5 rounded-lg" style={{ border: "1px solid var(--planning-border-soft)", color: "var(--text-muted)" }}>Overslaan →</button>}
            </div>
      ...
      case 7: {
        ...
        return (
          <div className="space-y-3">
            <div className="flex items-center justify-between">
              <h3 className="text-base font-bold" style={{ color: "var(--text-primary)" }}>Aansluitingen & OV</h3>
              {allZero && <button onClick={() => setStep(s => s + 1)} className="text-xs px-3 py-1.5 rounded-lg" style={{ border: "1px solid var(--planning-border-soft)", color: "var(--text-muted)" }}>Overslaan →</button>}
            </div>
      ...
      case 8: {
        ...
        return (
          <div className="space-y-3">
            <div className="flex items-center justify-between">
              <h3 className="text-base font-bold" style={{ color: "var(--text-primary)" }}>Vrijschakelen</h3>
              {allZero && <button onClick={() => setStep(s => s + 1)} className="text-xs px-3 py-1.5 rounded-lg" style={{ border: "1px solid var(--planning-border-soft)", color: "var(--text-muted)" }}>Overslaan →</button>}
            </div>
      ...
      case 9:
        return (
          <div className="space-y-4">
            <h3 className="text-base font-bold" style={{ color: "var(--text-primary)" }}>Aarding & revisie</h3>
            <div>
              <p className="text-xs font-semibold uppercase tracking-wider mb-2" style={{ color: "var(--text-muted)" }}>Aarding</p>
            ...
            <div style={{ borderTop: "1px solid var(--planning-border-soft)", paddingTop: 12 }}>
              <p className="text-xs font-semibold uppercase tracking-wider mb-2" style={{ color: "var(--text-muted)" }}>Opleverdossier</p>
              <div className="space-y-2">
                {([
                  ...
                  <OptionButton key={o.val} selected={answers.revisie === o.val} onClick={() => set({ revisie: o.val })}>
                    <div className="flex items-center gap-2">
                      ...
                      <div>
                        <p className="text-sm font-medium">{o.label}</p>
                        {o.code && <p className="text-[11px]" style={{ color: "var(--text-muted)", fontFamily: "DM Mono, monospace" }}>{o.code} — {euro(getSpec(o.code)?.tarief || 0)}</p>}
                        {o.hint && <p className="text-[11px]" style={{ color: "var(--text-muted)" }}>{o.hint}</p>}
                      </div>
                    </div>
                  </OptionButton>
                ))}
      ...
      case 10: {
        ...
        return (
          <div className="space-y-4">
            <h3 className="text-base font-bold" style={{ color: "var(--text-primary)" }}>WV & personeel</h3>
            <div>
              ...
              {answers.wv && (
                <div className="ml-4 animate-fade-in">
                  ...
                  <p className="text-[11px] italic" style={{ color: "var(--text-muted)" }}>Standaard {defaultWv}u voor {project.case_type || "dit type"}</p>
                </div>
              )}
            </div>
            <div>
            ...
            <div style={{ borderTop: "1px solid var(--planning-border-soft)", paddingTop: 12 }}>
              <p className="text-xs font-semibold uppercase tracking-wider mb-2" style={{ color: "var(--text-muted)" }}>Extra personeel (optioneel)</p>
      ...
      case 11:
        return (
          <div className="space-y-3">
            <div>
              <h3 className="text-base font-bold" style={{ color: "var(--text-primary)" }}>Overzicht forecast</h3>
              <p className="text-xs" style={{ color: "var(--text-muted)" }}>Controleer en pas aan indien nodig</p>
            </div>
            {overzichtRegels.length === 0 ? (
              <p className="text-sm text-center py-6" style={{ color: "var(--text-muted)" }}>Geen codes geselecteerd. Ga terug en vul de stappen in.</p>
            ) : (
              <div className="rounded-xl overflow-hidden" style={{ border: "1px solid var(--planning-border-soft)" }}>
                {groepen.map(([groep, regels]) => {
                  ...
                  return (
                    <div key={groep}>
                      <button onClick={() => setOpenGroups(prev => { const n = new Set(prev); if (n.has(groep)) n.delete(groep); else n.add(groep); return n; })} className="w-full flex items-center justify-between px-3 py-2" style={{ background: "var(--bg-surface-2)" }}>
                        <span className="text-[11px] font-bold uppercase tracking-wider" style={{ color: "var(--text-muted)", letterSpacing: "0.5px" }}>
                          {GROEP_LABELS[groep] || groep} ({regels.length})
                        </span>
                        <div className="flex items-center gap-2">
                          <span className="text-[11px] font-medium" style={{ color: "var(--text-muted)", fontFamily: "DM Mono, monospace" }}>{euroShort(groepOmzet)}</span>
                      ...
                      {isOpen && regels.map(r => (
                        <div key={r.spec_code} className="flex items-center gap-2 px-3 py-2" style={{ borderTop: "1px solid var(--planning-border-soft)" }}>
                          <span className="text-[10px] font-bold shrink-0 w-16" style={{ color: "var(--text-muted)", fontFamily: "DM Mono, monospace" }}>{r.spec_code}</span>
                          <span className="text-xs flex-1 min-w-0 truncate" style={{ color: "var(--text-primary)" }}>{r.label}</span>
                          <div className="flex items-center gap-0 shrink-0">
                            <button onClick={() => { const nv = Math.max(r.min_aantal, r.aantal - 1); setAnswers(a => { const key = getAnswerKey(r.spec_code, a); if (key) return { ...a, [key]: nv }; return a; }); }} className="w-6 h-6 rounded-l flex items-center justify-center" style={{ background: "var(--app-navy)", border: "1px solid var(--planning-border-soft)" }}>
                              <Minus className="h-3 w-3" style={{ color: "var(--text-muted)" }} />
                            </button>
                            <div className="w-8 h-6 flex items-center justify-center text-[11px] font-bold" style={{ background: "var(--app-navy)", borderTop: "1px solid var(--planning-border-soft)", borderBottom: "1px solid var(--planning-border-soft)", fontFamily: "DM Mono, monospace", color: "var(--text-primary)" }}>{r.aantal}</div>
                            <button onClick={() => { const nv = Math.min(r.max_aantal, r.aantal + 1); setAnswers(a => { const key = getAnswerKey(r.spec_code, a); if (key) return { ...a, [key]: nv }; return a; }); }} className="w-6 h-6 rounded-r flex items-center justify-center" style={{ background: "var(--app-navy)", border: "1px solid var(--planning-border-soft)" }}>
                              <Plus className="h-3 w-3" style={{ color: "var(--text-muted)" }} />
                            </button>
                          </div>
                          <span className="text-[10px] shrink-0 w-10 text-right" style={{ color: "var(--text-muted)", fontFamily: "DM Mono, monospace" }}>{euro(r.tarief)}</span>
                          <span className="text-[11px] font-bold shrink-0 w-16 text-right" style={{ color: "var(--accent)", fontFamily: "DM Mono, monospace" }}>{euroShort(r.tarief * r.aantal)}</span>
                        </div>
                      ))}
                    </div>
                  );
                ...
                {/* Totaal */}
                <div className="px-3 py-3 text-right" style={{ borderTop: "2px solid var(--accent-border)" }}>
                  <p className="text-[11px] uppercase tracking-wider" style={{ color: "var(--text-muted)" }}>Totaal omzet (Van Gelder)</p>
                  <p className="text-lg font-bold" style={{ color: "var(--accent)", fontFamily: "DM Mono, monospace" }}>{euro(totaalOmzet)}</p>
                </div>
              </div>
            )}
          </div>
  ...
  if (confirmRedo) {
    return (
      <div className="fixed inset-0 z-50 flex items-end sm:items-center justify-center" style={{ background: "rgba(0,0,0,0.5)" }}>
        <div className="w-full sm:max-w-md rounded-t-2xl sm:rounded-2xl p-6 space-y-4" style={{ background: "var(--app-navy)", border: "1px solid var(--warn-border)" }}>
          <div className="flex items-start gap-3">
            <div className="w-10 h-10 rounded-full flex items-center justify-center shrink-0" style={{ background: "var(--warn-light)" }}>
              <AlertTriangle className="h-5 w-5" style={{ color: "var(--warn-text)" }} />
            </div>
            <div className="flex-1">
              <h3 className="text-base font-bold" style={{ color: "var(--text-primary)" }}>Bestaande forecast overschrijven?</h3>
          ...
          <div className="flex gap-2 pt-2">
            <button onClick={onClose} className="flex-1 px-4 py-2.5 rounded-xl text-sm font-medium" style={{ border: "1px solid var(--planning-border-soft)", color: "var(--text-muted)" }}>
              Annuleren
            </button>
            <button onClick={() => setConfirmRedo(false)} className="flex-1 px-4 py-2.5 rounded-xl text-sm font-bold text-white flex items-center justify-center gap-1" style={{ background: "var(--warn-text)" }}>
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-end sm:items-center justify-center" style={{ background: "rgba(0,0,0,0.5)" }}>
      <div className="w-full sm:max-w-lg max-h-[92vh] flex flex-col rounded-t-2xl sm:rounded-2xl" style={{ background: "var(--app-navy)" }}>
        {/* Header */}
        <div className="shrink-0 px-4 py-3 flex items-center justify-between" style={{ borderBottom: "1px solid var(--planning-border-soft)" }}>
          <div>
            <p className="text-sm font-bold" style={{ color: "var(--text-primary)" }}>Forecast intake</p>
            <p className="text-[11px]" style={{ color: "var(--text-muted)" }}>{project.nummer} · {project.naam}</p>
          </div>
          <button onClick={onClose} className="w-8 h-8 rounded-lg flex items-center justify-center" style={{ background: "var(--bg-surface)" }}>
        ...
        <div className="shrink-0 px-4 py-2">
          <div className="flex gap-1">
            {STAPPEN.map((_, i) => (
              <div key={i} className="flex-1 h-1 rounded-full" style={{ background: i < step ? "var(--accent)" : i === step ? "var(--accent)" : "var(--bg-surface-2)" }} />
            ))}
          </div>
          <p className="text-[11px] mt-1.5" style={{ color: "var(--text-muted)" }}>
            Stap {step + 1} van {totalSteps} — <span className="font-semibold" style={{ color: step === totalSteps - 1 ? "var(--accent)" : "var(--text-muted)" }}>{STAPPEN[step]}</span>
          </p>
        </div>
        ...
        {/* Navigation */}
        <div className="shrink-0 px-4 py-3 flex gap-2" style={{ borderTop: "1px solid var(--planning-border-soft)" }}>
          {step > 0 && (
            <button onClick={() => setStep(s => s - 1)} className="px-4 py-2.5 rounded-xl text-sm font-medium flex items-center gap-1" style={{ border: "1px solid var(--planning-border-soft)", color: "var(--text-muted)" }}>
              <ChevronLeft className="h-4 w-4" /> Vorige
            </button>
          )}
          <div className="flex-1" />
          {isOverzicht ? (
            <button onClick={handleSave} disabled={saving || overzichtRegels.length === 0} className="px-6 py-2.5 rounded-xl text-sm font-bold text-white flex items-center gap-2 disabled:opacity-50" style={{ background: "linear-gradient(135deg, var(--accent), var(--accent-dark))" }}>
              {saving ? <Loader2 className="h-4 w-4 animate-spin" /> : <FileText className="h-4 w-4" />}
              Forecast aanmaken
            </button>
          ) : (
            <button onClick={() => setStep(s => s + 1)} className="px-6 py-2.5 rounded-xl text-sm font-bold text-white flex items-center gap-1" style={{ background: "var(--accent)" }}>
