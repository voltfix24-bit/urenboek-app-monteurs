    ...
    <div className="rounded-xl p-3 mb-2" style={{ background: "var(--bg-surface)", border: "1px solid var(--border)" }}>
      <div className="flex items-center justify-between">
        ...
        <div className="flex items-center gap-0 shrink-0 ml-3">
          <button onClick={() => onChange(Math.max(min, value - step))} className="w-8 h-8 rounded-l-lg flex items-center justify-center" style={{ background: "var(--bg-base)", border: "1px solid var(--border)" }}>
            <Minus className="h-3.5 w-3.5" style={{ color: "var(--text-muted)" }} />
          </button>
          <div className="w-12 h-8 flex items-center justify-center text-sm font-bold" style={{ background: "var(--bg-base)", borderTop: "1px solid var(--border)", borderBottom: "1px solid var(--border)", color: "var(--text-primary)", fontFamily: "DM Mono, monospace" }}>
            {value}
          </div>
          <button onClick={() => onChange(Math.min(max, value + step))} className="w-8 h-8 rounded-r-lg flex items-center justify-center" style={{ background: "var(--bg-base)", border: "1px solid var(--border)" }}>
...
function OptionButton({ selected, onClick, children, color }: { selected: boolean; onClick: () => void; children: React.ReactNode; color?: string }) {
  return (
    <button onClick={onClick} className="w-full text-left rounded-xl p-3 transition-all" style={{
      background: selected ? (color === "warn" ? "var(--warn-bg)" : "var(--accent-light)") : "var(--bg-surface)",
      border: `1.5px solid ${selected ? (color === "warn" ? "var(--warn-border)" : "var(--accent-border)") : "var(--border)"}`,
...
function ToggleRow({ checked, onChange, label, hint }: { checked: boolean; onChange: (v: boolean) => void; label: string; hint?: string }) {
  return (
    <div className="flex items-center justify-between rounded-xl p-3 mb-1.5" style={{ background: "var(--bg-surface)", border: "1px solid var(--border)" }}>
      <div>
...
export function ForecastIntakeWizard({ projectId, project, onClose, onComplete }: Props) {
          ...
          <div className="space-y-3">
            ...
            {answers.rmu_vervangen && (
              <div className="space-y-3 animate-fade-in">
                <p className="text-xs font-semibold" style={{ color: "var(--text-secondary)" }}>Welk merk?</p>
                ...
                {answers.rmu_merk && (
                  <div className="space-y-2 animate-fade-in">
                    <p className="text-xs font-semibold" style={{ color: "var(--text-secondary)" }}>Configuratie</p>
                    <div className="flex flex-wrap gap-2">
                      {merkConfigs.map(c => (
                        <button key={c.id} onClick={() => { set({ rmu_configuratie_id: c.id, rmu_velden: c.velden }); }} className="px-3 py-2 rounded-lg text-xs font-medium transition-all" style={{
                          ...
                          border: `1.5px solid ${answers.rmu_configuratie_id === c.id ? "var(--accent-border)" : "var(--border)"}`,
                          color: "var(--text-primary)",
                        }}>
                          <span className="font-bold">{c.code}</span> · {c.velden}-velds
                        </button>
                      ))}
                      <button onClick={() => setShowNewRmu(true)} className="px-3 py-2 rounded-lg text-xs" style={{ border: "1px dashed var(--border)", color: "var(--text-muted)" }}>+ Niet in lijst</button>
                    </div>

                    {showNewRmu && (
                      <div className="flex gap-2 items-end p-3 rounded-xl" style={{ background: "var(--bg-surface)", border: "1px solid var(--border)" }}>
                    ...
                    {selectedRmu && (
                      <div className="rounded-xl p-3 animate-fade-in" style={{ background: "var(--success-light)", border: "1px solid var(--success-border)" }}>
                        <p className="text-xs font-semibold" style={{ color: "var(--success)" }}>Op basis van {selectedRmu.merk} {selectedRmu.code} ({selectedRmu.velden}-velds):</p>
        ...
        return (
          <div className="space-y-3">
            <div className="flex items-center justify-between">
              ...
              {allZero && <button onClick={() => setStep(s => s + 1)} className="text-xs px-3 py-1.5 rounded-lg" style={{ border: "1px solid var(--border)", color: "var(--text-muted)" }}>Overslaan →</button>}
            </div>
        ...
        return (
          <div className="space-y-3">
            ...
            {answers.rmu_vervangen && selectedRmu && (
              <div className="rounded-xl p-3" style={{ background: "var(--info-light)", border: "1px solid var(--info-border, var(--border))" }}>
                ...
                <div className="flex gap-2 mt-2">
                  ...
                  <button className="px-3 py-1.5 rounded-lg text-xs" style={{ border: "1px solid var(--border)", color: "var(--text-secondary)" }}>Zelf invullen</button>
                </div>
              </div>
            )}
            {isProvisiorium && (
              <div className="flex gap-2 p-3 rounded-r-xl" style={{ background: "var(--warn-bg)", borderLeft: "3px solid var(--warn-text)" }}>
                  ...
                  return (
                    <div key={groep}>
                      <button onClick={() => setOpenGroups(prev => { const n = new Set(prev); if (n.has(groep)) n.delete(groep); else n.add(groep); return n; })} className="w-full flex items-center justify-between px-3 py-2" style={{ background: "var(--bg-surface-2)" }}>
                        ...
                        <div className="flex items-center gap-2">
                          <span className="text-[11px] font-medium" style={{ color: "var(--text-secondary)", fontFamily: "DM Mono, monospace" }}>{euroShort(groepOmzet)}</span>
                      ...
                      {isOpen && regels.map(r => (
                        <div key={r.spec_code} className="flex items-center gap-2 px-3 py-2" style={{ borderTop: "1px solid var(--border)" }}>
                          ...
                          <div className="flex items-center gap-0 shrink-0">
                            <button onClick={() => { const nv = Math.max(r.min_aantal, r.aantal - 1); setAnswers(a => { const key = getAnswerKey(r.spec_code, a); if (key) return { ...a, [key]: nv }; return a; }); }} className="w-6 h-6 rounded-l flex items-center justify-center" style={{ background: "var(--bg-base)", border: "1px solid var(--border)" }}>
                              <Minus className="h-3 w-3" style={{ color: "var(--text-muted)" }} />
                            </button>
                            <div className="w-8 h-6 flex items-center justify-center text-[11px] font-bold" style={{ background: "var(--bg-base)", borderTop: "1px solid var(--border)", borderBottom: "1px solid var(--border)", fontFamily: "DM Mono, monospace", color: "var(--text-primary)" }}>{r.aantal}</div>
                            <button onClick={() => { const nv = Math.min(r.max_aantal, r.aantal + 1); setAnswers(a => { const key = getAnswerKey(r.spec_code, a); if (key) return { ...a, [key]: nv }; return a; }); }} className="w-6 h-6 rounded-r flex items-center justify-center" style={{ background: "var(--bg-base)", border: "1px solid var(--border)" }}>
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-end sm:items-center justify-center" style={{ background: "rgba(0,0,0,0.5)" }}>
      <div className="w-full sm:max-w-lg max-h-[92vh] flex flex-col rounded-t-2xl sm:rounded-2xl" style={{ background: "var(--bg-base)" }}>
        {/* Header */}
        ...
        <div className="shrink-0 px-4 py-2">
          <div className="flex gap-1">
            {STAPPEN.map((_, i) => (
              <div key={i} className="flex-1 h-1 rounded-full" style={{ background: i < step ? "var(--success)" : i === step ? "var(--accent)" : "var(--bg-surface-2)" }} />
            ))}
          </div>
          <p className="text-[11px] mt-1.5" style={{ color: "var(--text-muted)" }}>
            Stap {step + 1} van {totalSteps} — <span className="font-semibold" style={{ color: step === totalSteps - 1 ? "var(--accent)" : "var(--text-secondary)" }}>{STAPPEN[step]}</span>
          </p>
        </div>
