...
export function GlobalSearch({ onClose }: { onClose: () => void }) {
  ...
  return (
    <div className="fixed inset-0 z-[100] flex items-start justify-center pt-[15vh]" onClick={onClose}>
      <div className="absolute inset-0" style={{ background: "color-mix(in srgb, #1f2937 40%, transparent)", backdropFilter: "blur(8px)" }} />
      <div className="relative w-full rounded-2xl overflow-hidden" style={{ maxWidth: 560, maxHeight: "70vh", background: "#ffffff", border: "1px solid #e5e7eb" }} onClick={e => e.stopPropagation()}>
        <div className="flex items-center gap-3 px-4 py-3" style={{ borderBottom: "1px solid #e5e7eb" }}>
          <Search className="h-5 w-5 shrink-0" style={{ color: "#6b7280" }} />
          <input ref={inputRef} value={query} onChange={e => setQuery(e.target.value)} onKeyDown={handleKeyDown} placeholder="Zoek projecten, medewerkers, mededelingen..." className="flex-1 text-base bg-transparent outline-none" style={{ color: "#1f2937" }} />
          <button onClick={onClose}><X className="h-5 w-5" style={{ color: "#6b7280" }} /></button>
        </div>
        <div className="overflow-y-auto" style={{ maxHeight: "calc(70vh - 56px)" }}>
          {!query.trim() ? (
            <p className="text-center py-8 text-sm" style={{ color: "#6b7280" }}>Typ om te zoeken...</p>
          ) : results.length === 0 ? (
            <p className="text-center py-8 text-sm" style={{ color: "#6b7280" }}>Geen resultaten voor '{query}'</p>
          ) : (
            sectionOrder.map(type => {
              ...
              return (
                <div key={type}>
                  <p className="px-4 pt-3 pb-1 text-[10px] font-semibold uppercase tracking-wider" style={{ color: "#6b7280" }}>{sectionLabels[type]}</p>
                  {items.map(r => {
                    const gi = results.indexOf(r);
                    return (
                      <button key={r.id} onClick={() => handleSelect(r)} className="w-full flex items-center gap-3 px-4 py-2.5 text-left transition-colors" style={{ background: activeIndex === gi ? "#ecfdf5" : "transparent" }}>
                        ...
                        <div className="min-w-0 flex-1">
                          ...
                          {r.subtitle && <p className="text-[11px] truncate" style={{ color: "#6b7280" }}>{r.subtitle}</p>}
                        </div>
                      </button>
                    );
                  })}
