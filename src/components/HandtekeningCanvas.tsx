...
export function HandtekeningCanvas({ onSave, bestaande, readonly = false, hoogte = 150 }: Props) {
  ...
  return (
    <div className="space-y-2">
      <div className="rounded-xl overflow-hidden" style={{ border: '1px solid var(--planning-border-soft)', background: '#fff' }}>
        <canvas
          ref={canvasRef}
      ...
      {!readonly && (
        <div className="flex gap-2">
          <button onClick={wissen} className="flex-1 py-2 rounded-lg text-xs font-medium" style={{ border: '1px solid var(--planning-border-soft)', color: 'var(--text-muted)', background: 'var(--bg-surface)' }}>
            ✕ Wissen
          </button>
          <button onClick={opslaan} className="flex-1 py-2 rounded-lg text-xs font-medium" style={{ background: isEmpty ? 'var(--bg-surface-2)' : 'var(--accent)', color: isEmpty ? 'var(--text-muted)' : 'var(--on-accent)', border: 'none', opacity: isEmpty ? 0.5 : 1 }} disabled={isEmpty}>
