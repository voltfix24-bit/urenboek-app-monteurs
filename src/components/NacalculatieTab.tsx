...
const margeKleur = (p: number) => p >= 30 ? "#10b981" : p >= 15 ? "#d97706" : "#dc2626";
const margeBg = (p: number) => p >= 30 ? "#ecfdf5" : p >= 15 ? "rgba(254,179,0,0.1)" : "rgba(255,113,108,0.1)";
...
export function NacalculatieTab({ projectId }: Props) {
  ...
  return (
    <div className="space-y-5">
      ...
      <div className="rounded-[20px] p-5" style={{ background: "#ffffff", border: "1px solid #e5e7eb" }}>
        <p className="text-[11px] font-semibold uppercase tracking-wider mb-4" style={{ color: "#6b7280" }}>
          Financieel resultaat
        </p>

        {/* Omzet */}
        <div className="mb-4">
          <p className="text-[11px] uppercase tracking-wider mb-0.5" style={{ color: "#6b7280" }}>Omzet</p>
          <p className="text-[11px]" style={{ color: "#6b7280" }}>(Van Gelder tarief)</p>
          <p className="text-2xl font-bold mt-1" style={{ ...mono, color: werkelijkOmzet > 0 ? "#10b981" : "#6b7280" }}>
        ...
        {/* Personeelskosten */}
        <div className="mb-4">
          <p className="text-[11px] uppercase tracking-wider mb-0.5" style={{ color: "#6b7280" }}>Personeelskosten</p>
          <p className="text-[11px]" style={{ color: "#6b7280" }}>(monteurs × uurtarief)</p>
          <p className="text-lg font-semibold mt-1" style={{ ...mono, color: "#1f2937" }}>
            − {euro(totaleKosten)}
          </p>
        </div>
        ...
        <div className="flex items-center justify-between">
          <div>
            <p className="text-[13px] font-bold" style={{ color: "#1f2937" }}>Resultaat</p>
        ...
        {!heeftBoekingen && heeftForecast && (
          <div className="mt-4 rounded-xl p-3 flex items-start gap-2" style={{ background: "var(--app-navy)", border: "1px solid #e5e7eb" }}>
            <Info className="h-3.5 w-3.5 shrink-0 mt-0.5" style={{ color: "#6b7280" }} />
            <p className="text-[11px]" style={{ color: "#6b7280" }}>Er zijn nog geen goedgekeurde uren voor dit project.</p>
          </div>
        )}
      </div>
      ...
      {/* Onzekere uren info */}
      {heeftOnzekereUren && (
        <div className="rounded-xl p-3 flex items-start gap-2" style={{ background: "var(--app-navy)", border: "1px solid #e5e7eb" }}>
          <Info className="h-3.5 w-3.5 shrink-0 mt-0.5" style={{ color: "#6b7280" }} />
              ...
              <thead>
                <tr style={{ borderBottom: "1px solid #e5e7eb" }}>
                  {["Monteur", "Uren goed", "Tarief", "Kosten"].map(h => (
                    <th key={h} className="text-left pb-2 px-2 font-semibold" style={{ color: "#6b7280", fontSize: 10, textTransform: "uppercase" }}>{h}</th>
                  ))}
                </tr>
              </thead>
              <tbody>
                {monteurs.map(m => {
                  ...
                  return (
                    <tr key={m.id} style={{ borderBottom: "1px solid color-mix(in srgb, #e5e7eb 50%, transparent)" }}>
                      <td className="py-2 px-2">
                        <span className="font-medium" style={{ color: "#1f2937" }}>{m.prof?.full_name || "Onbekend"}</span>
                        {extra > 0 && (
                      ...
                      <td className="py-2 px-2 font-semibold" style={{ ...mono, color: "#10b981" }}>{m.goed}u</td>
                      <td className="py-2 px-2" style={{ ...mono, color: "#6b7280" }}>€{m.prof?.uurtarief || 0}/u</td>
                      <td className="py-2 px-2 font-semibold" style={{ ...mono, color: "#1f2937" }}>{euro(kosten)}</td>
                    </tr>
                  );
                })}
              </tbody>
              <tfoot>
                <tr style={{ borderTop: "2px solid #a7f3d0" }}>
                  <td className="py-2 px-2 font-semibold" style={{ color: "#1f2937" }}>Totaal</td>
                  <td className="py-2 px-2 font-bold" style={{ ...mono, color: "#1f2937" }}>{totaalUrenGoed}u</td>
                  <td />
                  <td className="py-2 px-2 font-bold" style={{ ...mono, color: "#1f2937" }}>{euro(goedgekeurdeKosten)}</td>
                </tr>
              </tfoot>
            </table>
          </div>
            ...
            <table className="w-full text-xs">
              ...
              <tbody>
                {stuksRegels.map(r => {
                  ...
                  return (
                    <tr key={r.id} style={{ borderBottom: "1px solid color-mix(in srgb, #e5e7eb 50%, transparent)" }}>
                      <td className="py-2 px-2 font-semibold" style={{ ...mono, color: "#10b981" }}>{r.spec_code}</td>
                      <td className="py-2 px-2" style={{ color: "#1f2937" }}>{r.spec_omschrijving}</td>
                      <td className="py-2 px-2" style={{ ...mono, color: "#6b7280" }}>{a}×</td>
                      <td className="py-2 px-2" style={{ ...mono, color: "#6b7280" }}>{euro(Number(r.tarief) || 0)}</td>
                      <td className="py-2 px-2 font-semibold" style={{ ...mono, color: "#1f2937" }}>{euro(totaal)}</td>
                    </tr>
                  );
                })}
              </tbody>
              <tfoot>
                <tr style={{ borderTop: "2px solid #a7f3d0" }}>
                  <td colSpan={4} className="py-2 px-2 font-semibold" style={{ color: "#1f2937" }}>Totale omzet Van Gelder</td>
                  <td className="py-2 px-2 font-bold" style={{ ...mono, color: "#10b981" }}>{euro(forecastOmzet)}</td>
                </tr>
              </tfoot>
            </table>
          </div>
      ...
      {methode === "uren" && (
        <div className="rounded-xl p-3 flex items-start gap-2" style={{ background: "var(--app-navy)", border: "1px solid #e5e7eb" }}>
          <Info className="h-3.5 w-3.5 shrink-0 mt-0.5" style={{ color: "#6b7280" }} />
          <p className="text-[11px]" style={{ color: "#6b7280" }}>
            Dit project wordt vergoed op basis van uren. Verwachte omzet vastgelegd in de forecast: <strong style={{ color: "#1f2937" }}>{euro(verwachteOmzet)}</strong>
          </p>
        </div>
      )}

      {!heeftForecast && (
        <div className="rounded-xl p-4 text-center space-y-2" style={{ background: "var(--app-navy)", border: "1px solid #e5e7eb" }}>
          <p className="text-xs" style={{ color: "#6b7280" }}>Nog geen forecast ingevuld. Voeg een forecast toe om de omzet te berekenen.</p>
          <button className="inline-flex items-center gap-1 text-xs font-semibold" style={{ color: "#10b981" }}>
      ...
      {heeftForecast && (heeftBoekingen || forecastOmzet > 0) && (
        <div className="rounded-2xl p-4 space-y-3" style={{ background: "var(--app-navy)", border: "1px solid #e5e7eb" }}>
          <p className="text-[11px] font-semibold uppercase tracking-wider" style={{ color: "#6b7280" }}>
          ...
          <div className="overflow-x-auto">
            <table className="w-full text-xs">
              <thead>
                <tr style={{ borderBottom: "1px solid #e5e7eb" }}>
                  <th className="text-left pb-2 px-2" style={{ color: "#6b7280", fontSize: 10, textTransform: "uppercase", fontWeight: 600 }}> </th>
                  <th className="text-right pb-2 px-2" style={{ color: "#6b7280", fontSize: 10, textTransform: "uppercase", fontWeight: 600 }}>Forecast</th>
                  <th className="text-right pb-2 px-2" style={{ color: "#6b7280", fontSize: 10, textTransform: "uppercase", fontWeight: 600 }}>Werkelijk</th>
                  <th className="text-right pb-2 px-2" style={{ color: "#6b7280", fontSize: 10, textTransform: "uppercase", fontWeight: 600 }}> </th>
                </tr>
              </thead>
              <tbody>
                {[
                  ...
                  return (
                    <tr key={row.label} style={{ borderBottom: "1px solid color-mix(in srgb, #e5e7eb 50%, transparent)" }}>
                      <td className="py-2.5 px-2 font-medium" style={{ color: "#1f2937" }}>{row.label}</td>
                      <td className="py-2.5 px-2 text-right" style={{ ...mono, color: "#6b7280" }}>{euro(row.fc)}</td>
                      <td className="py-2.5 px-2 text-right font-bold" style={{ ...mono, color: "#1f2937" }}>{euro(row.wk)}</td>
                      <td className="py-2.5 px-2 text-right w-8">
                        {!gelijk && (
                          beter
                            ? <TrendingUp className="h-3.5 w-3.5 inline" style={{ color: "#10b981" }} />
                            : slechter
                              ? <TrendingDown className="h-3.5 w-3.5 inline" style={{ color: "#dc2626" }} />
                              : null
                        )}
                      </td>
                    </tr>
                ...
                {/* Marge row */}
                <tr>
                  <td className="py-2.5 px-2 font-medium" style={{ color: "#1f2937" }}>Marge</td>
                  <td className="py-2.5 px-2 text-right" style={{ ...mono, color: "#6b7280" }}>{forecastMargePerc.toFixed(1)}%</td>
