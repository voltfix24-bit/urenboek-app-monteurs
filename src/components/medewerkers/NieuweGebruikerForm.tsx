...
function FormSection({ title, children }: { title: string; children: React.ReactNode }) {
  return (
    <div className="space-y-2">
      <p className="text-[11px] font-bold uppercase tracking-wider" style={{ color: "#10b981" }}>{title}</p>
      {children}
    </div>
  );
}
...
export function NieuweGebruikerForm(props: any) {
  ...
  return (
    <div className="rounded-2xl p-4 space-y-4 animate-fade-in" style={{ background: "#ffffff", border: "1px solid #a7f3d0" }}>
      <p className="text-sm font-semibold" style={{ color: "#1f2937" }}>Nieuwe medewerker</p>
      <form onSubmit={onSubmit} className="space-y-4">
        ...
        <FormSection title="2. Functie & tarief">
          <div className="space-y-1">
            <label className="text-[11px] font-semibold uppercase tracking-wider" style={{ color: formErrors.role ? "#dc2626" : "#6b7280" }}>Rol *</label>
            <div className="flex gap-1.5 flex-wrap">
              {Object.entries(roleLabels).map(([value, label]) => (
                <button key={value} type="button" onClick={() => { setRole(value); ce("role"); }} className="px-3 py-2 rounded-xl text-xs font-semibold transition-colors" style={{
                  background: role === value ? "#ecfdf5" : "var(--app-navy)",
                  border: role === value ? "1px solid #a7f3d0" : "1px solid #e5e7eb",
                  color: role === value ? "#10b981" : "#6b7280",
                }}>{label}</button>
              ))}
            </div>
            {formErrors.role && <p className="text-[10px] font-medium" style={{ color: "#dc2626" }}>⚠ {formErrors.role}</p>}
          </div>
          <div className="grid grid-cols-2 gap-2">
            ...
            <div className="space-y-1">
              <label className="text-[11px] font-semibold uppercase tracking-wider" style={{ color: "#6b7280" }}>Contract einddatum</label>
          ...
          <div className="flex items-center gap-3">
            <label className="text-[11px] font-semibold uppercase tracking-wider" style={{ color: "#6b7280" }}>Rijbewijs</label>
            <button type="button" onClick={() => setRijbewijs(!rijbewijs)} className="w-10 h-6 rounded-full transition-colors relative" style={{ background: rijbewijs ? "#10b981" : "#ffffff" }}>
              <div className="w-4 h-4 rounded-full bg-white absolute top-1 transition-all" style={{ left: rijbewijs ? 22 : 4 }} />
            </button>
            <span className="text-xs" style={{ color: "#6b7280" }}>{rijbewijs ? "Ja" : "Nee"}</span>
          </div>
        </FormSection>
        ...
        <FormSection title="5. Account aanmaken">
          <div className="flex gap-2">
            <button type="button" onClick={() => setInviteMode("invite")} className="flex-1 p-3 rounded-xl text-left space-y-1" style={{
              background: inviteMode === "invite" ? "#ecfdf5" : "var(--app-navy)",
              border: inviteMode === "invite" ? "2px solid #10b981" : "1px solid #e5e7eb",
            }}>
              <p className="text-xs font-bold flex items-center gap-1" style={{ color: inviteMode === "invite" ? "#10b981" : "#1f2937" }}>
                <Mail className="h-3.5 w-3.5" /> Uitnodiging sturen
              </p>
              <p className="text-[10px]" style={{ color: "#6b7280" }}>Aanbevolen — monteur stelt zelf wachtwoord in</p>
            </button>
            <button type="button" onClick={() => setInviteMode("password")} className="flex-1 p-3 rounded-xl text-left space-y-1" style={{
              background: inviteMode === "password" ? "#ecfdf5" : "var(--app-navy)",
              border: inviteMode === "password" ? "2px solid #10b981" : "1px solid #e5e7eb",
            }}>
              <p className="text-xs font-bold flex items-center gap-1" style={{ color: inviteMode === "password" ? "#10b981" : "#1f2937" }}>
                <Key className="h-3.5 w-3.5" /> Wachtwoord instellen
              </p>
              <p className="text-[10px]" style={{ color: "#6b7280" }}>Voor direct toegang</p>
            </button>
          </div>

          {inviteMode === "invite" ? (
            <p className="text-[11px] p-2 rounded-lg" style={{ background: "var(--app-navy)", color: "#6b7280" }}>
            ...
            <div className="space-y-1">
              ...
              <div className="flex gap-1.5">
                <div className="relative flex-1">
                  ...
                  <button type="button" onClick={() => setShowPw(!showPw)} className="absolute right-2 top-1/2 -translate-y-1/2" style={{ color: "#6b7280" }}>
                    {showPw ? <EyeOff className="h-3.5 w-3.5" /> : <Eye className="h-3.5 w-3.5" />}
                  </button>
                </div>
                <button type="button" onClick={generatePassword} className="px-3 py-2.5 rounded-xl text-sm shrink-0" style={{ background: "var(--app-navy)", border: "1px solid #e5e7eb" }}>🎲</button>
                {password && (
                  <button type="button" onClick={() => { navigator.clipboard.writeText(password); toast.success("Gekopieerd!"); }} className="px-3 py-2.5 rounded-xl text-sm shrink-0" style={{ background: "var(--app-navy)", border: "1px solid #e5e7eb" }}>
        ...
        <button type="submit" disabled={loading} className="w-full py-3 rounded-xl text-sm font-bold text-white transition-colors active:scale-[0.98]" style={{ background: "linear-gradient(135deg, #10b981, #047857)" }}>
