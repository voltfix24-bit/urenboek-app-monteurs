            ...
            <div className="flex gap-1.5 flex-wrap">
              {Object.entries(roleLabels).map(([value, label]) => (
                <button key={value} type="button" onClick={() => { setRole(value); ce("role"); }} className="px-3 py-2 rounded-xl text-xs font-semibold transition-colors" style={{
                  background: role === value ? "var(--accent-light)" : "var(--app-navy)",
                  border: role === value ? "1px solid var(--accent-border)" : "1px solid var(--planning-border-soft)",
        ...
        <FormSection title="5. Account aanmaken">
          <div className="flex gap-2">
            <button type="button" onClick={() => setInviteMode("invite")} className="flex-1 p-3 rounded-xl text-left space-y-1" style={{
              background: inviteMode === "invite" ? "var(--accent-light)" : "var(--app-navy)",
              border: inviteMode === "invite" ? "2px solid var(--accent)" : "1px solid var(--planning-border-soft)",
            }}>
            ...
            <button type="button" onClick={() => setInviteMode("password")} className="flex-1 p-3 rounded-xl text-left space-y-1" style={{
              background: inviteMode === "password" ? "var(--accent-light)" : "var(--app-navy)",
              border: inviteMode === "password" ? "2px solid var(--accent)" : "1px solid var(--planning-border-soft)",
            }}>
          ...
          {inviteMode === "invite" ? (
            <p className="text-[11px] p-2 rounded-lg" style={{ background: "var(--app-navy)", color: "var(--text-muted)" }}>
            ...
            <div className="space-y-1">
              ...
              <div className="flex gap-1.5">
                ...
                <button type="button" onClick={generatePassword} className="px-3 py-2.5 rounded-xl text-sm shrink-0" style={{ background: "var(--app-navy)", border: "1px solid var(--planning-border-soft)" }}>🎲</button>
                {password && (
                  <button type="button" onClick={() => { navigator.clipboard.writeText(password); toast.success("Gekopieerd!"); }} className="px-3 py-2.5 rounded-xl text-sm shrink-0" style={{ background: "var(--app-navy)", border: "1px solid var(--planning-border-soft)" }}>
