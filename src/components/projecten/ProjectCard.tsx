...
export function ProjectCard({ project, ogNaam, isManager, isEditing, isExpanded, isConfirmingDelete, isDefinitief, renderFormFields, onEdit, onCancel, onSave, onToggle, onDelete, onCancelDelete, onToggleExpand }: any) {
  if (isEditing) {
    return (
      <div className="rounded-2xl p-4 space-y-3 animate-fade-in" style={{ background: "var(--bg-surface)", border: "1px solid rgba(110,155,255,0.3)" }}>
        <h3 className="text-sm font-semibold" style={{ color: "var(--text-primary)" }}>Project bewerken</h3>
        {renderFormFields()}
        <div className="flex gap-2 pt-1">
          <button onClick={onCancel} className="flex-1 py-2 rounded-xl text-xs font-semibold flex items-center justify-center gap-1" style={{ background: "var(--app-navy)", color: "var(--text-muted)" }}><X className="h-3.5 w-3.5" /> Annuleren</button>
  ...
  return (
    <div className="rounded-2xl overflow-hidden transition-transform active:scale-[0.985]" style={{ background: "var(--bg-surface)", border: "1px solid var(--planning-border-soft)", opacity: project.active ? 1 : 0.5 }}>
      <div className="p-4 flex items-center gap-3 cursor-pointer" onClick={onToggleExpand}>
          ...
          <div className="flex items-center gap-2 flex-wrap">
            ...
            {isManager && (
              <span className="text-[10px] font-bold px-2 py-0.5 rounded-full" style={{ 
                background: isDefinitief ? "var(--accent-light)" : "var(--warn-light)", 
                border: `1px solid ${isDefinitief ? "var(--accent-border)" : "var(--warn-border)"}`,
                color: isDefinitief ? "var(--accent)" : "var(--warn-text)" 
              }}>
        ...
        <div className="flex items-center gap-1.5 shrink-0" onClick={e => e.stopPropagation()}>
          <button onClick={onEdit} className="w-8 h-8 rounded-lg flex items-center justify-center" style={{ background: "var(--bg-surface-2)" }}><Pencil className="h-3.5 w-3.5" style={{ color: "var(--text-muted)" }} /></button>
          <button onClick={onToggle} className="w-8 h-8 rounded-lg flex items-center justify-center" style={{ background: project.active ? "var(--accent-light)" : "var(--bg-surface-2)" }}>
            {project.active ? <ToggleRight className="h-4 w-4" style={{ color: "var(--accent)" }} /> : <ToggleLeft className="h-4 w-4" style={{ color: "var(--text-muted)" }} />}
          </button>
      ...
      {isExpanded && (
        <div className="px-4 pb-4 space-y-2 animate-fade-in" style={{ borderTop: "1px solid var(--planning-border-soft)" }}>
          ...
          {isManager && (project.contactpersoon_naam || project.contactpersoon_tel || project.contactpersoon_email) && (
            <div className="rounded-xl p-3 space-y-1.5 mt-2" style={{ background: "var(--warn-light)", border: "1px solid var(--warn-border)" }}>
          ...
          {isManager && (
            <div className="mt-3 pt-3" style={{ borderTop: "1px solid var(--planning-border-soft)" }}>
          ...
          <button onClick={() => generateProjectPdf(project, ogNaam, isManager)} className="w-full mt-2 py-2 rounded-xl text-xs font-semibold flex items-center justify-center gap-1.5" style={{ background: "var(--app-navy)", border: "1px solid var(--planning-border-soft)", color: "var(--text-muted)" }}>
