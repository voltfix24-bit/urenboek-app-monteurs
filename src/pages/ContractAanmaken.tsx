          ...
          <div className="rounded-2xl p-4 space-y-3" style={{ background: "var(--bg-surface)", border: "1px solid var(--planning-border-soft)" }}>
            ...
            <div>
              ...
              <select value={ogVertegenwoordiger} onChange={e => setOgVertegenwoordiger(e.target.value)}
                className="w-full mt-1 rounded-lg p-2.5 text-sm" style={{ background: "var(--app-navy)", border: "1px solid var(--planning-border-soft)", color: "var(--text-primary)" }}>
                {managers.map(m => (
      ...
      {stap === 2 && (
        <div className="space-y-4">
          ...
          <div className="flex gap-2">
            ...
            <button onClick={() => setStap(3)} className="flex-1 py-3 rounded-xl text-sm font-semibold" style={{ background: "var(--accent)", color: "var(--on-accent)" }}>Volgende →</button>
          </div>
        </div>
      )}
      ...
      {stap === 3 && (
        <div className="space-y-4">
          ...
          <div className="rounded-2xl p-4 space-y-3" style={{ background: "var(--bg-surface)", border: "1px solid var(--planning-border-soft)" }}>
            <div className="flex items-center justify-between">
              ...
              <button
                ...
                style={{ color: "var(--info)", border: "1px solid rgba(110,155,255,0.3)" }}>
                Volledig bewerken
              </button>
            </div>
            <div className="grid grid-cols-2 gap-2">
              <div>
                <label className="text-[11px] font-medium" style={{ color: "var(--text-muted)" }}>Uurtarief (€)</label>
                <Input type="number" value={uurtarief} onChange={e => setUurtarief(e.target.value)} className="mt-1" />
              </div>
              <div>
                <label className="text-[11px] font-medium" style={{ color: "var(--text-muted)" }}>Ingangsdatum</label>
            ...
            <div>
              <label className="text-[11px] font-medium" style={{ color: "var(--text-muted)" }}>Naam opdrachtnemer</label>
              <Input value={otNaam} onChange={e => setOtNaam(e.target.value)} className="mt-1" />
            </div>
            <div className="grid grid-cols-2 gap-2">
              <div>
                <label className="text-[11px] font-medium" style={{ color: "var(--text-muted)" }}>KVK opdrachtnemer</label>
                <Input value={otKvk} onChange={e => setOtKvk(e.target.value)} className="mt-1" />
              </div>
              <div>
                <label className="text-[11px] font-medium" style={{ color: "var(--text-muted)" }}>Plaats ondertekening</label>
                <Input value={plaats} onChange={e => setPlaats(e.target.value)} className="mt-1" />
              </div>
            </div>
          </div>
          ...
          <div className="rounded-2xl p-4 space-y-3" style={{ background: "var(--bg-surface)", border: "1px solid var(--planning-border-soft)" }}>
            ...
            <div className="max-h-[400px] overflow-y-auto pr-2 space-y-3 text-xs" style={{ color: "var(--text-muted)" }}>
              ...
              <hr style={{ borderColor: "var(--planning-border-soft)" }} />
...
function SectieHeader({ children }: { children: React.ReactNode }) {
  return (
    <p className="text-[11px] font-bold uppercase tracking-wider pb-1.5 mb-1" style={{ color: "var(--text-muted)", borderBottom: "1px solid var(--planning-border-soft)" }}>
      {children}
    </p>
  );
}
