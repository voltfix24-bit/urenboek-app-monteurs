...
function ContractAttentionSection({ navigate }: { navigate: (p: string) => void }) {
  ...
  return (
    <div className="rounded-2xl p-4 space-y-2" style={{ background: "var(--bg-surface)", border: "1px solid var(--planning-border-soft)" }}>
      ...
      {correcties.map(c => (
        <div key={c.id} className="flex items-center gap-2.5 p-2.5 rounded-xl" style={{ background: "var(--app-navy)", border: "1px solid var(--danger-border)" }}>
      ...
      {wachtend.map(c => (
        <div key={c.id} className="flex items-center gap-2.5 p-2.5 rounded-xl" style={{ background: "var(--app-navy)", border: "1px solid var(--planning-border-soft)" }}>
      ...
      {medewerkers.map(m => (
        <div key={m.id} className="flex items-center justify-between py-2" style={{ borderBottom: "1px solid var(--bg-surface-2)" }}>
          <div className="flex items-center gap-2">
            <div className="w-7 h-7 rounded-full flex items-center justify-center text-[10px] font-bold" style={{ background: "var(--accent)", color: "var(--on-accent)" }}>
              {m.full_name?.charAt(0)?.toUpperCase() || "?"}
            </div>
            <div>
              <p className="text-sm font-medium" style={{ color: "var(--text-primary)" }}>{m.full_name}</p>
              <span className="text-[10px] font-semibold px-2 py-0.5 rounded-full" style={{ background: "var(--accent-light)", color: "var(--accent)", border: "1px solid var(--accent-border)" }}>Klaar voor verificatie</span>
            </div>
          </div>
          ...
          {loading ? (
            ...
            <>
              ...
              <section style={{ marginBottom: 24 }}>
                ...
                <p style={{ fontSize: 14, color: "var(--text-muted)", fontFamily: "Hanken Grotesk" }}>
              ...
              {pendingCount > 0 && (
                <div onClick={() => navigate("/goedkeuring")} style={{
                  ...
                  cursor: "pointer", boxShadow: "0 4px 20px var(--warn-light)",
                }}>
                  <div style={{ display: "flex", alignItems: "center", gap: 12 }}>
                    <div style={{ width: 40, height: 40, borderRadius: "50%", background: "var(--warn-text)", display: "flex", alignItems: "center", justifyContent: "center" }}>
                      <span className="material-symbols-outlined" style={{ fontSize: 20, color: "#523700", fontVariationSettings: "'FILL' 1" }}>priority_high</span>
                    </div>
                    <span style={{ fontSize: 14, fontWeight: 700, fontFamily: "Hanken Grotesk", color: "var(--warn-text)" }}>
                      {pendingCount} weekstaten wachten op goedkeuring
                    </span>
                  </div>
                  <div style={{ display: "flex", alignItems: "center", gap: 4, background: "var(--warn-text)", borderRadius: 9999, padding: "6px 14px" }}>
                    <span style={{ fontSize: 12, fontWeight: 700, color: "#523700", fontFamily: "Hanken Grotesk" }}>Bekijk</span>
              ...
              <div style={{ display: "grid", gridTemplateColumns: "repeat(3, 1fr)", gap: 12, marginBottom: 24 }}>
                {[
                  { label: "Monteurs", value: teamCount ?? "—", color: "var(--accent)", icon: "engineering" },
                  { label: "Openstaand", value: pendingCount ?? "—", color: "var(--warn-text)", icon: "pending_actions" },
                  { label: "Deze week", value: weekHours ? `${weekHours}u` : "—", color: "var(--accent)", icon: "timer" },
                ].map((stat) => (
                  <div key={stat.label} style={{ background: "var(--bg-surface-2)", borderRadius: 16, padding: "16px 12px", borderBottom: `2px solid ${stat.color}40` }}>
                    <p style={{ fontSize: 9, fontWeight: 700, fontFamily: "Hanken Grotesk", textTransform: "uppercase", letterSpacing: "0.1em", color: "var(--text-muted)", marginBottom: 6 }}>
                      {stat.label}
                    </p>
                    <div style={{ display: "flex", alignItems: "flex-end", justifyContent: "space-between" }}>
                      <span style={{ fontFamily: "Hanken Grotesk", fontWeight: 800, fontSize: 24, color: stat.color }}>{stat.value}</span>
              ...
              {todayPlanning.length > 0 && (
                <section style={{ marginBottom: 24 }}>
                  <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 12 }}>
                    <span style={{ fontSize: 10, fontWeight: 700, fontFamily: "Hanken Grotesk", textTransform: "uppercase", letterSpacing: "0.2em", color: "var(--accent)" }}>
                      VANDAAG INGEPLAND
                    </span>
                  ...
                  <div style={{ display: "flex", flexDirection: "column", gap: 2 }}>
                    {todayPlanning.map((m: any) => {
                      ...
                      const chipColor = hasBoeking ? "var(--accent)" : "var(--warn-text)";
                      const chipBg = hasBoeking ? "var(--accent-light)" : "var(--warn-light)";
                        ...
                        <div key={m.id} style={{ background: "var(--bg-surface)", borderRadius: 12, padding: "14px 16px", display: "flex", alignItems: "center", justifyContent: "space-between" }}>
                          <div style={{ display: "flex", alignItems: "center", gap: 12 }}>
                            ...
                            <div>
                              ...
                              <span style={{ fontSize: 11, color: "var(--text-muted)", fontFamily: "Hanken Grotesk" }}>{m.project} · {m.starttijd}–{m.eindtijd}</span>
                            </div>
                          </div>
                          <div style={{ display: "flex", alignItems: "center", gap: 8 }}>
                            <div style={{ padding: "4px 10px", borderRadius: 9999, background: chipBg, border: `1px solid ${chipColor}50` }}>
                              <span style={{ fontSize: 9, fontWeight: 700, fontFamily: "Hanken Grotesk", textTransform: "uppercase", color: chipColor }}>{chipLabel}</span>
                            </div>
              ...
              {pendingEntries.length > 0 && (
                <section style={{ marginBottom: 24 }}>
                  <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 12 }}>
                    <span style={{ fontSize: 10, fontWeight: 700, fontFamily: "Hanken Grotesk", textTransform: "uppercase", letterSpacing: "0.2em", color: "var(--text-muted)" }}>
                    ...
                    {pendingEntries.slice(0, 5).map((e: any, i: number) => (
                      <div key={e.id} style={{
                        ...
                        <div style={{ display: "flex", alignItems: "flex-start", gap: 14 }}>
                          ...
                          <div>
                            <p style={{ fontSize: 13, fontWeight: 700, color: "var(--text-primary)", fontFamily: "Hanken Grotesk", marginBottom: 2 }}>{e.full_name}</p>
                        ...
                        <div style={{ display: "flex", gap: 6, flexShrink: 0 }}>
                          <button onClick={() => handleApprove(e.id)} style={{ width: 32, height: 32, borderRadius: 8, display: "flex", alignItems: "center", justifyContent: "center", background: "var(--accent-light)", border: "1px solid var(--accent-border)", cursor: "pointer" }}>
                            <span className="material-symbols-outlined" style={{ fontSize: 18, color: "var(--accent)", fontVariationSettings: "'FILL' 1" }}>check</span>
                          </button>
                          <button onClick={() => { setAfkeurId(e.id); setAfkeurReden(""); }} style={{ width: 32, height: 32, borderRadius: 8, display: "flex", alignItems: "center", justifyContent: "center", background: "var(--danger-light)", border: "1px solid var(--danger-border)", cursor: "pointer" }}>
                            <span className="material-symbols-outlined" style={{ fontSize: 18, color: "var(--danger)", fontVariationSettings: "'FILL' 1" }}>close</span>
                          </button>
                        </div>
                      </div>
                    ))}
                  ...
                  {afkeurId && (
                    <div style={{ display: "flex", gap: 8, marginTop: 12 }}>
                      <input value={afkeurReden} onChange={ev => setAfkeurReden(ev.target.value)} placeholder="Reden voor afkeuring..." autoFocus onKeyDown={ev => ev.key === "Enter" && handleReject()}
                        style={{ flex: 1, padding: "10px 14px", borderRadius: 12, fontSize: 13, background: "var(--bg-surface)", border: "1px solid var(--danger-border)", color: "var(--text-primary)", fontFamily: "Hanken Grotesk", outline: "none" }}
                      />
                      <button onClick={handleReject} style={{ padding: "10px 16px", borderRadius: 12, fontSize: 13, fontWeight: 700, color: "#fff", background: "var(--danger)", border: "none", cursor: "pointer", fontFamily: "Hanken Grotesk" }}>Afkeuren</button>
                      <button onClick={() => setAfkeurId(null)} style={{ padding: "10px", borderRadius: 12, fontSize: 16, color: "var(--text-muted)", background: "none", border: "none", cursor: "pointer" }}>×</button>
                    </div>
                  )}
                </section>
              )}
                    ...
                    {verlofAanvragen.map((v: any, i: number) => (
                      <div key={v.id} style={{ display: "flex", alignItems: "center", justifyContent: "space-between", padding: "14px 16px", borderBottom: i < verlofAanvragen.length - 1 ? "1px solid rgba(61,72,93,0.2)" : "none" }}>
                        ...
                        <div style={{ display: "flex", gap: 6 }}>
                          <button onClick={() => handleVerlof(v.id, "goedgekeurd")} style={{ width: 32, height: 32, borderRadius: 8, display: "flex", alignItems: "center", justifyContent: "center", background: "var(--accent-light)", border: "1px solid var(--accent-border)", cursor: "pointer" }}>
                            <span className="material-symbols-outlined" style={{ fontSize: 18, color: "var(--accent)", fontVariationSettings: "'FILL' 1" }}>check</span>
                          </button>
                          <button onClick={() => handleVerlof(v.id, "afgekeurd")} style={{ width: 32, height: 32, borderRadius: 8, display: "flex", alignItems: "center", justifyContent: "center", background: "var(--danger-light)", border: "1px solid var(--danger-border)", cursor: "pointer" }}>
                            <span className="material-symbols-outlined" style={{ fontSize: 18, color: "var(--danger)", fontVariationSettings: "'FILL' 1" }}>close</span>
                          </button>
                        </div>
                      </div>
              ...
              {overurenMeldingen.length > 0 && (
                <section style={{ marginBottom: 24 }}>
                  <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 12 }}>
                    <span style={{ fontSize: 10, fontWeight: 700, fontFamily: "Hanken Grotesk", textTransform: "uppercase", letterSpacing: "0.2em", color: "var(--danger)" }}>
                    ...
                    {overurenMeldingen.map((m: any, i: number) => (
                      <div key={m.id} style={{ display: "flex", alignItems: "center", justifyContent: "space-between", padding: "14px 16px", borderBottom: i < overurenMeldingen.length - 1 ? "1px solid rgba(61,72,93,0.2)" : "none" }}>
                        <div style={{ display: "flex", alignItems: "flex-start", gap: 14 }}>
                          ...
                          <div>
                            <p style={{ fontSize: 13, fontWeight: 700, color: "var(--text-primary)", fontFamily: "Hanken Grotesk", marginBottom: 2 }}>{m.full_name}</p>
                            <p style={{ fontSize: 11, color: "var(--text-muted)", fontFamily: "Hanken Grotesk" }}>
                    ...
                    {projectsWithMarge.map((p: any, i: number) => (
                      <div key={p.id} style={{ display: "flex", alignItems: "center", justifyContent: "space-between", padding: "14px 16px", borderBottom: i < projectsWithMarge.length - 1 ? "1px solid rgba(61,72,93,0.2)" : "none" }}>
                        ...
                        <div style={{
                          ...
                          background: p.marge >= 30 ? "var(--accent-light)" : p.marge >= 15 ? "var(--warn-light)" : "var(--danger-light)",
                          border: `1px solid ${p.marge >= 30 ? "var(--accent-border)" : p.marge >= 15 ? "var(--warn-border)" : "var(--danger-border)"}`,
                        }}>
                          <span style={{ fontSize: 12, fontWeight: 700, fontFamily: "Hanken Grotesk", color: p.marge >= 30 ? "var(--accent)" : p.marge >= 15 ? "var(--warn-text)" : "var(--danger)" }}>
                            {p.marge.toFixed(1)}%
                          </span>
                        </div>
                      </div>
                    ...
                    {zonderOrder.map((m) => {
                      ...
                      return (
                        <div key={m.id} onClick={() => navigate(`/inkooporders?medewerker=${m.id}`)} style={{ background: "var(--bg-surface)", borderRadius: 12, padding: "14px 16px", display: "flex", alignItems: "center", justifyContent: "space-between", cursor: "pointer" }}>
                          ...
                          <div style={{ display: "flex", alignItems: "center", gap: 4, background: "var(--accent)", borderRadius: 9999, padding: "6px 14px" }}>
                            <span style={{ fontSize: 12, fontWeight: 700, color: "var(--accent-dark)", fontFamily: "Hanken Grotesk" }}>Order</span>
                            <span className="material-symbols-outlined" style={{ fontSize: 14, color: "var(--accent-dark)" }}>arrow_forward</span>
                          </div>
                        </div>
                      );
                    })}
                      ...
                      return (
                        <div key={c.id} style={{ display: "flex", alignItems: "center", justifyContent: "space-between", padding: "14px 16px", borderBottom: i < expiringCerts.length - 1 ? "1px solid rgba(61,72,93,0.2)" : "none" }}>
                          <div style={{ display: "flex", alignItems: "flex-start", gap: 14 }}>
                            ...
                            <div>
                              <p style={{ fontSize: 13, fontWeight: 700, color: "var(--text-primary)", fontFamily: "Hanken Grotesk", marginBottom: 2 }}>{c.medewerker}</p>
                              <p style={{ fontSize: 11, color: "var(--text-muted)", fontFamily: "Hanken Grotesk" }}>{c.naam} ({c.type})</p>
                            </div>
                          </div>
                          <span style={{ fontSize: 11, fontWeight: 700, color: expired ? "var(--danger)" : "var(--warn-text)", fontFamily: "Hanken Grotesk" }}>
              ...
              {pendingEntries.length === 0 && verlofAanvragen.length === 0 && todayPlanning.length === 0 && projectsWithMarge.length === 0 && (
                <div style={{ textAlign: "center", padding: "48px 20px", background: "var(--bg-surface)", borderRadius: 16 }}>
                  <span className="material-symbols-outlined" style={{ fontSize: 40, color: "var(--accent)", marginBottom: 8, display: "block", fontVariationSettings: "'FILL' 1" }}>check_circle</span>
                  <p style={{ fontSize: 15, fontWeight: 700, color: "var(--text-primary)", fontFamily: "Hanken Grotesk", marginBottom: 4 }}>Alles bijgewerkt</p>
                  <p style={{ fontSize: 13, color: "var(--text-muted)", fontFamily: "Hanken Grotesk" }}>Geen openstaande items</p>
                </div>
              )}
            </>
          )}
