  ...
  const renderSimpleEditForm = () => (
    <div className="mx-6 lg:mx-10 mb-4 rounded-[14px] p-5 space-y-4" style={{ background: "var(--accent-light)", border: "1.5px solid var(--accent-border)" }}>
      ...
      <div className="flex gap-2 pt-1">
        <button onClick={() => setEditId(null)} className="flex-1 py-2.5 rounded-xl text-sm font-medium" style={{ border: "1px solid var(--border)", color: "var(--text-secondary)" }}>Annuleren</button>
  ...
  return (
    <>
      ...
      <div className="lg:ml-[240px] min-h-screen" style={{ background: "var(--bg-base)" }}>
        <header className="flex items-center gap-3 px-6 lg:px-10 pt-6 pb-4">
          ...
          <div className="flex rounded-lg overflow-hidden" style={{ border: "1px solid var(--border)" }}>
            <button onClick={() => setViewMode("simple")} className="px-3 py-1.5 text-xs font-medium flex items-center gap-1.5" style={{
              ...
              color: viewMode === "simple" ? "var(--accent)" : "var(--text-secondary)",
            ...
            <button onClick={() => setViewMode("advanced")} className="px-3 py-1.5 text-xs font-medium flex items-center gap-1.5" style={{
              ...
              color: viewMode === "advanced" ? "var(--accent)" : "var(--text-secondary)",
        ...
        {/* ─── Simulator ─── */}
        {showSim && (
          <div className="mx-6 lg:mx-10 mb-4 rounded-[14px] p-4 space-y-4" style={{ background: "var(--bg-surface)", border: "1px solid var(--border)" }}>
            <p className="text-sm font-semibold" style={{ color: "var(--text-primary)" }}>🧪 Scenario testen</p>
            <div className="space-y-3">
              <div>
                <p className="text-xs font-medium mb-1.5" style={{ color: "var(--text-secondary)" }}>Wat voor case?</p>
                <div className="flex gap-2 flex-wrap">
                  {["NSA-case", "Compactstation", "Provisorium", "Overig"].map(ct => (
                    <button key={ct} onClick={() => setSimCaseType(ct === "Overig" ? "" : ct)} className="px-3 py-2 rounded-xl text-xs font-medium" style={{
                      background: (ct === "Overig" ? !simCaseType : simCaseType === ct) ? "var(--accent-light)" : "var(--bg-base)",
                      border: `1.5px solid ${(ct === "Overig" ? !simCaseType : simCaseType === ct) ? "var(--accent-border)" : "var(--border)"}`,
              ...
              <div>
                <p className="text-xs font-medium mb-1.5" style={{ color: "var(--text-secondary)" }}>RMU vervangen?</p>
                <div className="flex gap-2">
                  <button onClick={() => setSimRmu(true)} className="px-4 py-2 rounded-xl text-xs font-medium" style={{ background: simRmu ? "var(--accent-light)" : "var(--bg-base)", border: `1.5px solid ${simRmu ? "var(--accent-border)" : "var(--border)"}`, color: "var(--text-primary)" }}>Ja</button>
                  <button onClick={() => setSimRmu(false)} className="px-4 py-2 rounded-xl text-xs font-medium" style={{ background: !simRmu ? "var(--accent-light)" : "var(--bg-base)", border: `1.5px solid ${!simRmu ? "var(--accent-border)" : "var(--border)"}`, color: "var(--text-primary)" }}>Nee</button>
                  {simRmu && (
                    <div className="flex items-center gap-2 ml-2">
                      <span className="text-xs" style={{ color: "var(--text-muted)" }}>Velden:</span>
                      {[3, 4, 5, 6].map(v => (
                        <button key={v} onClick={() => setSimVelden(v)} className="w-8 h-8 rounded-lg text-xs font-bold" style={{ background: simVelden === v ? "var(--accent-light)" : "var(--bg-base)", border: `1.5px solid ${simVelden === v ? "var(--accent-border)" : "var(--border)"}`, color: "var(--text-primary)" }}>{v}</button>
                      ))}
                    </div>
                  )}
                </div>
              ...
              <div className="grid grid-cols-2 gap-3">
                <div>
                  <p className="text-xs font-medium mb-1.5" style={{ color: "var(--text-secondary)" }}>Trafo?</p>
                  <div className="flex gap-1.5 flex-wrap">
                    {([["Nieuw", "nieuw"], ["Draaien", "draaien"], ["Vrijschakelen", "vrijschakelen"], ["Geen", "geen"]] as const).map(([l, v]) => (
                      <button key={v} onClick={() => setSimTrafo(v)} className="px-3 py-1.5 rounded-lg text-[11px] font-medium" style={{ background: simTrafo === v ? "var(--accent-light)" : "var(--bg-base)", border: `1px solid ${simTrafo === v ? "var(--accent-border)" : "var(--border)"}`, color: "var(--text-primary)" }}>{l}</button>
                    ))}
                  </div>
                </div>
                <div>
                  <p className="text-xs font-medium mb-1.5" style={{ color: "var(--text-secondary)" }}>LS-rek?</p>
                  <div className="flex gap-1.5 flex-wrap">
                    {([["≤630kVA", "klein"], [">630kVA", "groot"], ["Uitbreiden", "uitbreiden"], ["Geen", "geen"]] as const).map(([l, v]) => (
                      <button key={v} onClick={() => setSimLsRek(v)} className="px-3 py-1.5 rounded-lg text-[11px] font-medium" style={{ background: simLsRek === v ? "var(--accent-light)" : "var(--bg-base)", border: `1px solid ${simLsRek === v ? "var(--accent-border)" : "var(--border)"}`, color: "var(--text-primary)" }}>{l}</button>
                    ))}
                  </div>
                </div>
              </div>
            ...
            {simResult.length > 0 && (
              <div className="rounded-[14px] overflow-hidden" style={{ background: "var(--bg-surface)", border: "1px solid var(--border)" }}>
        ...
        {viewMode === "simple" && (
          <div className="px-6 lg:px-10 pb-24 space-y-3">
            {loading ? (
              ...
              return (
                <div key={cat} className="rounded-[14px] overflow-hidden" style={{ background: "var(--bg-surface)", border: "1px solid var(--border)" }}>
                  ...
                  {isOpen && catRegels.map(r => (
                    <div key={r.id} className="flex items-start gap-3 px-4 py-3" style={{ borderTop: "1px solid var(--border)", opacity: r.actief ? 1 : 0.5, background: r.actief ? undefined : "var(--bg-base)" }}>
                      ...
                      <div className="flex items-center gap-2 shrink-0 pt-1">
                        <button onClick={() => startEdit(r)} className="w-7 h-7 rounded-lg flex items-center justify-center" style={{ background: "var(--bg-base)", border: "1px solid var(--border)" }}>
              ...
              {loading ? (
                ...
                <div key={r.id} className="grid grid-cols-12 gap-1 px-3.5 py-2.5 items-center group" style={{ borderTop: "1px solid var(--border)", background: "var(--bg-surface)", opacity: r.actief ? 1 : 0.5 }}>
                  ...
                  <div className="col-span-2 flex items-center justify-end gap-1">
                    <button onClick={() => startEdit(r)} className="w-7 h-7 rounded-lg flex items-center justify-center opacity-0 group-hover:opacity-100 transition-opacity" style={{ background: "var(--bg-base)", border: "1px solid var(--border)" }}><Pencil className="h-3 w-3" style={{ color: "var(--text-muted)" }} /></button>
                    {confirmDeleteId === r.id ? (
                      <div className="flex gap-1">
                        <button onClick={() => setConfirmDeleteId(null)} className="w-7 h-7 rounded-lg flex items-center justify-center" style={{ background: "var(--bg-base)", border: "1px solid var(--border)" }}><X className="h-3 w-3" style={{ color: "var(--text-muted)" }} /></button>
