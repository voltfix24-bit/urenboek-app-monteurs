...
export default function Medewerkers() {
  ...
  if (!isManager) return <div className="min-h-screen flex items-center justify-center" style={{ background: "var(--app-navy)" }}><p style={{ color: "var(--text-muted)" }}>Alleen managers hebben toegang.</p></div>;
    ...
    return (
      ...
      <PageShell>
        <div style={{ background: "var(--app-navy)", minHeight: "100dvh", paddingBottom: "calc(env(safe-area-inset-bottom, 34px) + 100px)" }}>
          <header className="mobile-safe-header" style={{ position: "sticky", top: 0, zIndex: 50, background: "color-mix(in srgb, var(--bg-surface) 94%, transparent)", backdropFilter: "blur(20px)", borderBottom: "1px solid var(--planning-border-soft)", padding: "12px 20px", display: "flex", alignItems: "center", gap: 12 }}>
            <button onClick={() => setSelectedEmployee(null)} style={{ background: "none", border: "none", cursor: "pointer", color: "var(--accent)", display: "flex" }}>
              <ArrowLeft size={24} />
            </button>
          ...
          <main style={{ padding: "24px 20px" }}>
            {/* HERO CARD */}
            <div style={{ background: "var(--bg-surface)", borderRadius: 24, padding: "32px 24px", marginBottom: 20, display: "flex", flexDirection: "column", alignItems: "center", position: "relative", overflow: "hidden", border: "1px solid var(--planning-border-soft)" }}>
              <div style={{ position: "absolute", top: -40, left: "50%", transform: "translateX(-50%)", width: 160, height: 160, borderRadius: "50%", background: "radial-gradient(circle, var(--accent-light), transparent)", pointerEvents: "none" }} />
              <div style={{ width: 72, height: 72, borderRadius: "50%", background: "var(--accent)", display: "flex", alignItems: "center", justifyContent: "center", fontFamily: "Hanken Grotesk", fontWeight: 800, fontSize: 24, color: "var(--accent-dark)", marginBottom: 16, boxShadow: "0 0 30px var(--accent-border)", position: "relative", zIndex: 1 }}>
                {selectedEmployee.full_name?.split(" ").map((n: string) => n[0]).slice(0, 2).join("")}
              </div>
              <h2 style={{ fontFamily: "Hanken Grotesk", fontWeight: 800, fontSize: 22, color: "var(--text-primary)", marginBottom: 8, position: "relative", zIndex: 1 }}>{selectedEmployee.full_name}</h2>
              <div style={{ padding: "4px 14px", borderRadius: 9999, background: "var(--accent-light)", border: "1px solid var(--accent-border)", marginBottom: 6, position: "relative", zIndex: 1 }}>
                <span style={{ fontSize: 11, fontWeight: 700, fontFamily: "Hanken Grotesk", color: "var(--accent)", textTransform: "uppercase", letterSpacing: "0.05em" }}>
                  {roleLabels[selectedEmployee.role] || selectedEmployee.role}
                </span>
              </div>
              <span style={{ fontSize: 12, color: "var(--text-muted)", fontFamily: "Hanken Grotesk", position: "relative", zIndex: 1 }}>TerreVolt BV</span>
            </div>
  ...
  return (
    ...
    <PageShell>
      <div style={{ background: "var(--app-navy)", minHeight: "100dvh", paddingBottom: "calc(env(safe-area-inset-bottom, 34px) + 120px)" }}>
        {/* HEADER */}
        <header className="mobile-safe-header" style={{ position: "sticky", top: 0, zIndex: 50, background: "color-mix(in srgb, var(--bg-surface) 94%, transparent)", backdropFilter: "blur(20px)", borderBottom: "1px solid var(--planning-border-soft)", padding: "12px 20px", display: "flex", alignItems: "center", justifyContent: "space-between" }}>
          <div style={{ display: "flex", alignItems: "center", gap: 8 }}>
            <span className="material-symbols-outlined" style={{ color: "var(--accent)", fontSize: 24, fontVariationSettings: "'FILL' 1" }}>bolt</span>
            <span style={{ fontFamily: "Hanken Grotesk", fontWeight: 800, fontSize: 18, color: "var(--accent)", letterSpacing: "0.1em", textTransform: "uppercase" }}>TERREVOLT UREN</span>
          </div>
        ...
        <main style={{ padding: "24px 20px" }}>
          ...
          <section style={{ marginBottom: 20 }}>
            <p style={{ fontSize: 10, fontWeight: 700, fontFamily: "Hanken Grotesk", textTransform: "uppercase", letterSpacing: "0.2em", color: "var(--accent)", marginBottom: 4 }}>TEAM OVERZICHT</p>
            <div style={{ display: "flex", alignItems: "flex-end", justifyContent: "space-between" }}>
              <h2 style={{ fontFamily: "Hanken Grotesk", fontWeight: 800, fontSize: 26, color: "var(--text-primary)" }}>{employees.length} medewerkers</h2>
              <span style={{ fontSize: 13, color: "var(--text-muted)", fontFamily: "Hanken Grotesk" }}>Week {weekNum}</span>
            </div>
          </section>
          ...
          <div style={{ display: "flex", gap: 8, marginBottom: 20, overflowX: "auto", scrollbarWidth: "none" }}>
            {(["alle", "verificatie"] as const).map(f => (
              <button key={f} onClick={() => setFilter(f)} style={{
                ...
                fontFamily: "Hanken Grotesk", fontWeight: 700, fontSize: 12, cursor: "pointer", whiteSpace: "nowrap",
              }}>
          ...
          {showAdd && (
            <div style={{ marginBottom: 20 }}>
              <div style={{ background: "var(--warn-light)", border: "1px solid var(--warn-border)", borderRadius: 16, padding: "14px 16px", marginBottom: 12, display: "flex", alignItems: "flex-start", gap: 10 }}>
                <AlertTriangle size={16} style={{ color: "var(--warn-text)", marginTop: 2, flexShrink: 0 }} />
                <span style={{ fontSize: 12, fontFamily: "Hanken Grotesk", color: "var(--warn-text)" }}>Dit formulier is alleen bedoeld voor uitzonderingsgevallen. Nieuwe medewerkers worden normaal aangemaakt via <strong>Kandidaten → Contract → Ondertekenen</strong>.</span>
              </div>
          ...
          {createdUsers.length > 0 && (
            <div style={{ background: "var(--bg-surface)", border: "1px solid var(--accent-border)", borderRadius: 16, marginBottom: 20, overflow: "hidden" }}>
              <div style={{ padding: "10px 16px", background: "var(--accent-light)" }}>
                <span style={{ fontSize: 11, fontWeight: 700, fontFamily: "Hanken Grotesk", color: "var(--accent)", textTransform: "uppercase", letterSpacing: "0.1em" }}>Zojuist aangemaakt</span>
              </div>
                  ...
                  <div key={i} style={{ padding: 12, borderRadius: 12, background: "var(--app-navy)", border: "1px solid var(--border)" }}>
                    <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start", gap: 8 }}>
                      <div>
                        ...
                        <p style={{ fontSize: 11, color: "var(--text-muted)", fontFamily: "Hanken Grotesk" }}>{u.email} · {roleLabels[u.role]}</p>
                      </div>
                      {u.inviteOnly ? (
          ...
          {filter === "alle" && managers.length > 0 && (
            <div style={{ marginBottom: 16 }}>
              <p style={{ fontSize: 10, fontWeight: 700, fontFamily: "Hanken Grotesk", textTransform: "uppercase", letterSpacing: "0.2em", color: "var(--text-muted)", marginBottom: 10, paddingLeft: 2 }}>Managers ({managers.length})</p>
          ...
          {filteredMonteurs.length > 0 && (
            <div style={{ marginBottom: 16 }}>
              <p style={{ fontSize: 10, fontWeight: 700, fontFamily: "Hanken Grotesk", textTransform: "uppercase", letterSpacing: "0.2em", color: "var(--text-muted)", marginBottom: 10, paddingLeft: 2 }}>Medewerkers ({filteredMonteurs.length})</p>
              <div style={{ display: "flex", flexDirection: "column", gap: 10 }}>
                {filteredMonteurs.map((emp) => {
                  ...
                  const statusColor = isInactief ? "var(--text-muted)" : isOnboarding ? "var(--warn-text)" : "var(--accent)";
                  const statusLabel = isInactief ? "INACTIEF" : isOnboarding ? "ONBOARDING" : "ACTIEF";
                  const statusBg = isInactief ? "var(--bg-surface-2)" : isOnboarding ? "var(--warn-light)" : "var(--accent-light)";
                  return (
                      ...
                      <div style={{ display: "flex", alignItems: "center", gap: 14 }}>
                        ...
                        <div style={{ display: "flex", alignItems: "center", gap: 8 }}>
                          <div style={{ padding: "4px 10px", borderRadius: 9999, background: statusBg, border: `1px solid ${statusColor}50` }}>
                            <span style={{ fontSize: 9, fontWeight: 700, fontFamily: "Hanken Grotesk", textTransform: "uppercase", color: statusColor }}>{statusLabel}</span>
                          </div>
                          <span className="material-symbols-outlined" style={{ fontSize: 16, color: "var(--text-muted)" }}>chevron_right</span>
                        </div>
                      </div>
            ...
            <div style={{ background: "var(--bg-surface)", backdropFilter: "blur(12px)", borderRadius: 9999, padding: "10px 16px", display: "flex", justifyContent: "space-between", alignItems: "center", border: "1px solid var(--border)" }}>
              <div style={{ display: "flex", gap: 16 }}>
                {[
                  ...
                  { color: "var(--warn-text)", label: `${employees.filter(e => e.account_status === "onboarding" || e.account_status === "invited").length} onboarding` },
                  { color: "var(--text-muted)", label: `${employees.filter(e => e.account_status === "inactive").length} inactief` },
                ].map(s => (
                  <div key={s.label} style={{ display: "flex", alignItems: "center", gap: 4 }}>
                    <div style={{ width: 6, height: 6, borderRadius: "50%", background: s.color }} />
                    <span style={{ fontSize: 11, fontWeight: 700, color: "var(--text-primary)", fontFamily: "Hanken Grotesk" }}>{s.label}</span>
                  </div>
                ))}
              </div>
            </div>
