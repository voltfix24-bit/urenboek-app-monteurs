  ...
  return (
    <div className="min-h-screen flex flex-col" style={{ background: "var(--app-navy)" }}>
      ...
      <div className="flex items-center justify-between px-6 py-4">
        {step > 1 ? (
          <button onClick={() => setStep(step - 1)} className="flex items-center gap-1 text-sm" style={{ color: "var(--text-muted)" }}>
      ...
      <div className="flex-1 flex flex-col items-center px-6 pb-8" style={{ maxWidth: 440, margin: "0 auto", width: "100%" }}>
        ...
        {step === 1 && (
          <div className="w-full space-y-4 mt-6">
            ...
            <div className="rounded-2xl p-4 space-y-2" style={{ background: "var(--bg-surface)", border: "1px solid var(--planning-border-soft)" }}>
              <div className="flex items-center gap-2"><Check className="h-4 w-4" style={{ color: "var(--accent)" }} /><span className="text-sm" style={{ color: "var(--text-primary)" }}>Naam: {profileData?.full_name}</span></div>
              <div className="flex items-center gap-2"><Check className="h-4 w-4" style={{ color: "var(--accent)" }} /><span className="text-sm" style={{ color: "var(--text-primary)" }}>E-mail: {user?.email}</span></div>
              <div className="flex items-center gap-2"><Check className="h-4 w-4" style={{ color: "var(--accent)" }} /><span className="text-sm" style={{ color: "var(--text-primary)" }}>Rol: {roles[0] || "medewerker"}</span></div>
              {profileData?.telefoon && <div className="flex items-center gap-2"><Check className="h-4 w-4" style={{ color: "var(--accent)" }} /><span className="text-sm" style={{ color: "var(--text-primary)" }}>Telefoon: {profileData.telefoon}</span></div>}
            </div>
          ...
          <div className="w-full space-y-4 mt-6">
            ...
            <div className="space-y-3">
              <div className="space-y-1">
                ...
                <input value={form.telefoon} onChange={e => setForm({ ...form, telefoon: e.target.value })} placeholder="06-12345678" className="w-full px-3 py-2.5 rounded-xl text-sm" style={{ background: "var(--bg-surface)", border: "1px solid var(--planning-border-soft)", color: "var(--text-primary)" }} />
              </div>
              <div className="space-y-1">
                <label className="text-[11px] font-medium" style={{ color: "var(--text-muted)" }}>Adres</label>
                <input value={form.adres} onChange={e => setForm({ ...form, adres: e.target.value })} placeholder="Straat 1, Stad" className="w-full px-3 py-2.5 rounded-xl text-sm" style={{ background: "var(--bg-surface)", border: "1px solid var(--planning-border-soft)", color: "var(--text-primary)" }} />
              </div>
              ...
              <div className="space-y-1">
                ...
                <input value={form.noodcontact_naam} onChange={e => setForm({ ...form, noodcontact_naam: e.target.value })} placeholder="Naam" className="w-full px-3 py-2.5 rounded-xl text-sm" style={{ background: "var(--bg-surface)", border: "1px solid var(--planning-border-soft)", color: "var(--text-primary)" }} />
              </div>
              <div className="space-y-1">
                <label className="text-[11px] font-medium" style={{ color: "var(--text-muted)" }}>Noodcontact telefoon</label>
                <input value={form.noodcontact_tel} onChange={e => setForm({ ...form, noodcontact_tel: e.target.value })} placeholder="06-12345678" className="w-full px-3 py-2.5 rounded-xl text-sm" style={{ background: "var(--bg-surface)", border: "1px solid var(--planning-border-soft)", color: "var(--text-primary)" }} />
              </div>
            </div>
          ...
          <div className="w-full space-y-4 mt-6">
            ...
            <div className="space-y-3">
              <div className="space-y-1">
                ...
                <input value={zzpForm.bedrijfsnaam} onChange={e => setZzpForm({ ...zzpForm, bedrijfsnaam: e.target.value })} placeholder="Hassan el Garrat Elektrotechniek" className="w-full px-3 py-2.5 rounded-xl text-sm" style={{ background: "var(--bg-surface)", border: "1px solid var(--planning-border-soft)", color: "var(--text-primary)" }} />
              ...
              <div className="space-y-1">
                ...
                <input value={zzpForm.factuuradres} onChange={e => setZzpForm({ ...zzpForm, factuuradres: e.target.value })} placeholder="Zelfde als woonadres" className="w-full px-3 py-2.5 rounded-xl text-sm" style={{ background: "var(--bg-surface)", border: "1px solid var(--planning-border-soft)", color: "var(--text-primary)" }} />
                  ...
                  {[14, 21, 30].map(d => (
                    <button key={d} onClick={() => setZzpForm({ ...zzpForm, betalingstermijn: d })}
                      ...
                      style={{
                        ...
                        border: zzpForm.betalingstermijn === d ? "1.5px solid var(--accent-border)" : "1px solid var(--planning-border-soft)",
        ...
        {step === 5 && (
          <div className="w-full space-y-4 mt-6">
            ...
            <p className="text-sm" style={{ color: "var(--text-muted)" }}>Welke dagen ben je standaard niet beschikbaar?</p>
            <div className="flex gap-2">
              {DAGEN.map(d => {
                ...
                return (
                  <button key={d.key} onClick={() => setVrijeDagen(active ? vrijeDagen.filter(v => v !== d.key) : [...vrijeDagen, d.key])} className="flex-1 py-3 rounded-xl text-sm font-semibold" style={{
                    ...
                    border: active ? "1px solid var(--accent-border)" : "1px solid var(--planning-border-soft)",
