    ...
    <div className="min-h-screen" style={{ background: "var(--app-navy)" }}>
      <div className="mx-auto px-5 py-8 space-y-6" style={{ maxWidth: 480 }}>
        ...
        <div className="text-center space-y-3">
          ...
          <h1 className="text-xl font-bold" style={{ color: "#1f2937" }}>Welkom, {voornaam}! 👋</h1>
          <p className="text-sm" style={{ color: "#6b7280" }}>Vul je gegevens aan om aan de slag te kunnen.</p>
        </div>

        {/* Progress */}
        <div className="rounded-2xl p-4 space-y-3" style={{ background: "#ffffff", border: "1px solid #e5e7eb" }}>
          <div className="flex items-center justify-between">
            <span className="text-xs font-semibold" style={{ color: "#6b7280" }}>{voltooid} van 4 stappen voltooid</span>
            <span className="text-sm font-bold" style={{ color: "#10b981", fontFamily: "DM Mono, monospace" }}>{percentage}%</span>
          </div>
          <div className="w-full rounded-full overflow-hidden" style={{ height: 8, background: "#ffffff" }}>
            <div className="h-full rounded-full transition-all duration-500" style={{ width: `${percentage}%`, background: "#10b981" }} />
          </div>
        </div>
        ...
        <div className="space-y-3">
          {stappenData.map((s, i) => (
            <button key={i} onClick={() => navigate(s.link)} className="w-full flex items-center gap-3 p-4 rounded-2xl text-left transition-all active:scale-[0.98]" style={{
              background: s.compleet ? "#ecfdf5" : "#ffffff",
              border: s.compleet ? "1px solid #a7f3d0" : "1px solid #e5e7eb",
            }}>
              <div className="w-8 h-8 rounded-full flex items-center justify-center shrink-0" style={{
                background: s.compleet ? "#10b981" : "#ffffff",
                color: s.compleet ? "#fff" : "#6b7280",
              }}>
                {s.compleet ? <Check className="h-4 w-4" /> : <span className="text-xs font-bold">{i + 1}</span>}
              </div>
              <div className="flex-1 min-w-0">
                <p className="text-sm font-semibold" style={{ color: "#1f2937" }}>{s.label}</p>
          ...
          <div className="p-4 rounded-2xl space-y-3" style={{
            background: stap4 ? "#ecfdf5" : "#ffffff",
            border: stap4 ? "1px solid #a7f3d0" : "1px solid #e5e7eb",
            opacity: allesKlaar || stap4 ? 1 : 0.5,
          }}>
            <div className="flex items-center gap-3">
              <div className="w-8 h-8 rounded-full flex items-center justify-center shrink-0" style={{
                background: stap4 ? "#10b981" : "#ffffff",
                color: stap4 ? "#fff" : "#6b7280",
              }}>
                {stap4 ? <Check className="h-4 w-4" /> : <span className="text-xs font-bold">4</span>}
              </div>
              <div className="flex-1 min-w-0">
                <p className="text-sm font-semibold" style={{ color: "#1f2937" }}>Profiel afronden</p>
            ...
            {!stap4 && allesKlaar && (
              <button onClick={handleAfronden} disabled={afronden} className="w-full py-2.5 rounded-xl text-sm font-semibold disabled:opacity-50" style={{ background: "#10b981", color: "#fff" }}>
        ...
        {!profile.onboarding_voltooid ? (
          <div className="rounded-2xl p-4 flex items-start gap-3" style={{ background: "rgba(254,179,0,0.1)", border: "1px solid rgba(254,179,0,0.3)" }}>
            ...
            <div>
              <p className="text-sm font-semibold" style={{ color: "#d97706" }}>Je account is nog niet actief</p>
              <p className="text-xs mt-1" style={{ color: "#6b7280" }}>Vul alle stappen in zodat je manager je kan verifiëren.</p>
            </div>
          </div>
