          ...
          <div className="space-y-2">
            {items.map((item) => (
              confirmDeleteId === item.id ? (
                <div key={item.id} className="rounded-2xl p-4 space-y-3 animate-fade-in" style={{ background: "rgba(239,68,68,0.06)", border: "1px solid rgba(239,68,68,0.3)" }}>
                  <p className="text-sm font-semibold text-foreground">"{item.naam}" verwijderen?</p>
                  <div className="flex gap-2">
                    <button onClick={() => setConfirmDeleteId(null)} className="flex-1 py-2 rounded-xl text-xs font-semibold text-muted-foreground" style={{ background: "rgba(255,255,255,0.04)", border: "1px solid rgba(255,255,255,0.08)" }}>Annuleren</button>
                    <button onClick={() => handleDelete(item)} className="flex-1 py-2 rounded-xl text-xs font-bold text-white" style={{ background: "#ef4444" }}>Verwijderen</button>
                  </div>
                </div>
              ) : (
