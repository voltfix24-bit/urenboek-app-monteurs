    ...
    <PageShell>
      <header className="sticky top-0 z-30 px-4 py-3" style={{ background: "color-mix(in srgb, var(--bg-surface) 97%, transparent)", backdropFilter: "blur(12px)", borderBottom: "1px solid var(--planning-border-soft)" }}>
        <div className="flex items-center gap-3">
          ...
          <button onClick={() => { setShowAdd(true); setEditId(null); setForm(emptyForm); }} className="w-8 h-8 rounded-lg flex items-center justify-center" style={{ background: "var(--accent-light)", border: "1px solid var(--accent-border)" }}><Plus className="h-4 w-4" style={{ color: "var(--accent)" }} /></button>
        </div>
      </header>
      <div className="px-4 py-4 space-y-3">
        {(showAdd || editId) && (
          <div className="rounded-2xl p-4 space-y-3 animate-fade-in" style={{ background: "var(--bg-surface)", border: `1px solid ${editId ? "rgba(110,155,255,0.3)" : "var(--accent-border)"}` }}>
          ...
          <div className="space-y-2">
            {items.map(item => confirmDeleteId === item.id ? (
              <div key={item.id} className="rounded-2xl p-4 space-y-3 animate-fade-in" style={{ background: "var(--danger-light)", border: "1px solid var(--danger-border)" }}>
                ...
                <div className="flex gap-2">
                  <button onClick={() => setConfirmDeleteId(null)} className="flex-1 py-2 rounded-xl text-xs font-semibold" style={{ background: "var(--app-navy)", border: "1px solid var(--planning-border-soft)", color: "var(--text-muted)" }}>Annuleren</button>
              ...
              <div key={item.id} className="rounded-2xl p-4 transition-transform active:scale-[0.985]" style={{ background: "var(--bg-surface)", border: "1px solid var(--planning-border-soft)" }}>
