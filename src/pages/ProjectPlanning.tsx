...
export default function ProjectPlanning() {
  ...
  const renderCellPanel = () => {
    ...
    return (
      <div className="fixed top-0 right-0 h-full z-50 flex flex-col" style={{ width: 320, background: "var(--bg-surface)", borderLeft: "1px solid var(--planning-border-soft)", animation: "slideInRight 0.25s ease" }}>
        <div className="flex items-center justify-between p-4" style={{ borderBottom: "1px solid var(--planning-border-soft)" }}>
          <div>
          ...
          <button onClick={() => setSelectedCell(null)} className="w-8 h-8 rounded-lg flex items-center justify-center" style={{ color: "var(--text-muted)" }}><X className="h-4 w-4" /></button>
        </div>
        ...
        <div className="flex-1 overflow-y-auto p-4 space-y-5">
            ...
            <div className="space-y-1.5">
              {monteurs.map(m => (
                <button key={m.id} onClick={() => setCell(selectedCell, { medewerker_id: cell?.medewerker_id === m.id ? null : m.id })}
                  ...
                  style={{ background: cell?.medewerker_id === m.id ? "var(--accent-light)" : "var(--app-navy)", border: "1px solid var(--planning-border-soft)" }}>
                  <span className="w-2 h-2 rounded-full" style={{ background: cell?.medewerker_id === m.id ? "var(--accent)" : "var(--planning-border-soft)" }} />
          ...
          <div>
            ...
            <textarea rows={2} value={cell?.note || ""} onChange={e => setCell(selectedCell, { note: e.target.value })}
              ...
              style={{ background: "var(--app-navy)", border: "1px solid var(--planning-border-soft)", color: "var(--text-primary)" }} />
          </div>
        </div>

        <div className="p-4 space-y-2" style={{ borderTop: "1px solid var(--planning-border-soft)" }}>
  ...
  return (
    <div className="min-h-screen" style={{ background: "var(--app-navy)" }}>
      ...
      <header className="sticky top-0 z-40 px-4 py-3 flex items-center gap-3 flex-wrap" style={{ background: "color-mix(in srgb, var(--bg-surface) 97%, transparent)", backdropFilter: "blur(12px)", borderBottom: "1px solid var(--planning-border-soft)" }}>
        ...
        {/* Save status */}
        <span className="text-[11px]" style={{ color: saveStatus === "saving" ? "var(--text-muted)" : saveStatus === "saved" ? "var(--accent)" : "transparent" }}>
          {saveStatus === "saving" ? "Opslaan..." : saveStatus === "saved" ? "Opgeslagen" : "·"}
        </span>

        {/* Status badge */}
        <span className="px-2.5 py-1 rounded-full text-[11px] font-semibold flex items-center gap-1" style={{ background: isDef ? "var(--accent-light)" : "var(--warn-light)", color: isDef ? "var(--accent)" : "var(--warn-text)" }}>
          <span className="w-1.5 h-1.5 rounded-full" style={{ background: isDef ? "var(--accent)" : "var(--warn-text)" }} />
          {isDef ? "Definitief" : "Concept"}
        </span>

        <button onClick={() => setShowTemplates(true)} className="px-3 py-1.5 rounded-lg text-xs font-medium" style={{ border: "1px solid var(--planning-border-soft)", color: "var(--text-muted)" }}>
        ...
        {isDef ? (
          <button onClick={() => setShowConceptDialog(true)} className="px-4 py-1.5 rounded-lg text-xs font-semibold text-white flex items-center gap-1" style={{ background: "var(--accent)" }}>
      ...
      <div className="p-4 grid grid-cols-1 lg:grid-cols-3 gap-4">
          ...
          <div className="grid grid-cols-3 gap-3 text-xs">
            ...
            <div>
              ...
              <select value={state.trafo} onChange={e => updateState(s => ({ ...s, trafo: e.target.value }))}
                className="w-full px-2 py-1.5 rounded-lg text-xs" style={{ background: "var(--app-navy)", border: "1px solid var(--planning-border-soft)", color: "var(--text-primary)" }}>
              ...
              <div className="flex gap-1.5">
                {(["MSH", "LSH", "MSR", "LSR"] as const).map(k => (
                  <button key={k} onClick={() => updateState(s => ({ ...s, werkplanToggles: { ...s.werkplanToggles, [k]: !s.werkplanToggles[k] } }))}
                    ...
                    style={{ background: state.werkplanToggles[k] ? "var(--accent-light)" : "var(--bg-surface)", color: state.werkplanToggles[k] ? "var(--text-primary)" : "var(--text-muted)", border: "1px solid var(--planning-border-soft)" }}>
                    {k}
                  </button>
                ))}
              </div>
        ...
        {/* Notes */}
        <div className="rounded-2xl p-4" style={{ background: "var(--warn-light)", border: "1px solid var(--warn-border)" }}>
      ...
      {/* Matrix */}
      <div className="mx-4 mb-4 rounded-2xl overflow-hidden" style={{ background: "white", border: "1px solid var(--planning-border-soft)" }}>
        {/* Week headers - synced scroll */}
        <div className="flex" style={{ borderBottom: "1px solid var(--planning-border-soft)" }}>
          <div className="flex-shrink-0 flex items-center px-3 text-xs font-semibold" style={{ width: SIDEBAR_W, background: "var(--accent-light)", color: "var(--text-primary)", borderRight: "1px solid var(--planning-border-soft)" }}>
            Planning {state.year}
        ...
        <div className="flex">
          {/* Activities sidebar */}
          <div className="flex-shrink-0" style={{ width: SIDEBAR_W, borderRight: "1px solid var(--planning-border-soft)" }}>
            <div className="px-3 py-1 text-[10px] font-semibold uppercase tracking-wider" style={{ background: "hsl(120,25%,90%)", color: "var(--text-muted)", height: 30, display: "flex", alignItems: "center" }}>
              Activiteiten
            </div>
            {state.activities.map((act, ai) => (
              <div key={ai} draggable onDragStart={() => handleDragStart(ai)} onDragOver={e => handleDragOver(e, ai)} onDrop={() => handleDrop(ai)}
                className="flex items-center gap-1.5 px-2 group" style={{ height: CELL_SIZE, borderBottom: "1px solid var(--planning-border-soft)", cursor: "grab", opacity: dragIdx === ai ? 0.5 : 1 }}>
                <GripVertical className="h-3 w-3 shrink-0" style={{ color: "var(--planning-border-soft)" }} />
            ...
            <button onClick={() => updateState(s => ({ ...s, activities: [...s.activities, ""] }))}
              className="w-full text-center py-2 text-[11px] font-medium" style={{ color: "var(--accent)", borderBottom: "1px dashed var(--planning-border-soft)" }}>
          ...
          <div ref={bodyScrollRef} className="flex-1 overflow-x-auto" onScroll={() => syncScroll("body")}
            style={{ scrollbarWidth: "thin", scrollbarColor: "var(--planning-border-soft) var(--bg-surface)" }}>
                        ...
                        >
                          {monteur && (
                            <span className="absolute top-0.5 left-0.5 w-4 h-4 rounded-full flex items-center justify-center text-[7px] font-bold"
                              style={{ background: "var(--bg-surface)", color: cell?.color || "var(--text-primary)" }}>
        ...
        {/* Comments strip */}
        <div className="flex" style={{ borderTop: "1px solid var(--planning-border-soft)" }}>
          <div className="flex-shrink-0 px-3 flex items-center text-[10px] font-semibold uppercase tracking-wider" style={{ width: SIDEBAR_W, color: "var(--text-muted)", borderRight: "1px solid var(--planning-border-soft)", height: 44 }}>
            Opmerkingen
          </div>
              ...
              {state.weekNrs.map((wn, wi) => (
                <div key={wi} style={{ width: 5 * CELL_SIZE }}>
                  <textarea value={state.weekComments[wi] || ""} onChange={e => updateState(s => ({ ...s, weekComments: { ...s.weekComments, [wi]: e.target.value } }))}
                    ...
                    className="w-full bg-transparent text-[10px] px-2 py-1 resize-none outline-none" style={{ height: 44, color: "var(--text-primary)", borderRight: "1px solid var(--planning-border-soft)" }} />
                </div>
              ))}
            </div>
          </div>
        ...
        <div className="flex flex-wrap gap-3">
          {COLORS.map(c => (
            <div key={c.id} className="flex items-center gap-1">
              ...
              <span className="text-[9px]" style={{ color: "var(--text-muted)" }}>{c.name}</span>
            </div>
          ))}
        </div>
          ...
          <div className="px-4 py-2 space-y-1">
            ...
            {planningCostBreakdown.rows.map((row, i) => (
              <div key={i} className="flex items-center text-xs py-1" style={{ borderTop: i > 0 ? "1px solid color-mix(in srgb, var(--accent-border) 50%, transparent)" : "none" }}>
                ...
                <span className="w-16 text-right font-mono" style={{ color: "var(--text-muted)" }}>{row.days}d</span>
                <span className="w-20 text-right font-mono" style={{ color: "var(--text-muted)" }}>€{row.tarief}</span>
      ...
      {/* Context menu */}
      {contextMenu && (
        <div className="fixed z-50 rounded-xl p-3 space-y-3 shadow-lg" style={{ left: contextMenu.x, top: contextMenu.y, background: "var(--app-navy)", border: "1px solid var(--planning-border-soft)", width: 200 }}
  ...
  return (
    <div>
      ...
      <input value={value} readOnly={readonly} onChange={e => onChange?.(e.target.value)}
        className="w-full px-2 py-1.5 rounded-lg text-xs" style={{ background: readonly ? "var(--bg-surface-2)" : "var(--app-navy)", border: "1px solid var(--planning-border-soft)", color: "var(--text-primary)" }} />
    </div>
  );
}
...
function TemplateModal({ onClose, onApply }: { onClose: () => void; onApply: (acts: string[]) => void }) {
  ...
  const renderEditCard = (isNew: boolean) => (
    <div className="p-4 rounded-2xl space-y-3" style={{ background: "var(--accent-light)", border: "1.5px solid var(--accent-border)" }}>
      <input ref={isNew ? nameRef : undefined} value={editNaam} onChange={e => setEditNaam(e.target.value)} placeholder="Template naam"
        className="w-full text-sm font-bold rounded-lg px-3 py-2" style={{ background: "var(--app-navy)", border: "1px solid var(--planning-border-soft)", color: "var(--text-primary)" }} />
      <input value={editOmschrijving} onChange={e => setEditOmschrijving(e.target.value)} placeholder="Korte omschrijving"
        className="w-full text-xs rounded-lg px-3 py-1.5" style={{ background: "var(--app-navy)", border: "1px solid var(--planning-border-soft)", color: "var(--text-muted)" }} />
      <div>
        ...
        {editActiviteiten.map((a, i) => (
          <div key={i} className="flex items-center gap-2 mb-1.5"
            ...
            <input value={a} onChange={e => { const arr = [...editActiviteiten]; arr[i] = e.target.value; setEditActiviteiten(arr); }}
              className="flex-1 text-[13px] rounded-lg px-2.5 py-1.5" style={{ background: "var(--app-navy)", border: "1px solid var(--planning-border-soft)", color: "var(--text-primary)" }} placeholder="Activiteit naam" />
      ...
      <div className="flex gap-2 pt-1">
        <button onClick={cancelEdit} className="flex-1 py-1.5 rounded-lg text-xs" style={{ border: "1px solid var(--planning-border-soft)", color: "var(--text-muted)" }}>Annuleren</button>
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center" style={{ background: "rgba(0,0,0,0.4)" }}>
      <div className="rounded-2xl p-6 w-full max-w-lg space-y-4 max-h-[80vh] overflow-y-auto" style={{ background: "var(--app-navy)", border: "1px solid var(--planning-border-soft)" }}>
          ...
          <div className="flex items-center gap-2">
            {isManager && !manageMode && !editingId && (
              <>
                ...
                <button onClick={() => setManageMode(true)} className="px-2.5 py-1 rounded-lg text-[11px] font-medium" style={{ border: "1px solid var(--planning-border-soft)", color: "var(--text-muted)" }}>
            ...
            return (
              <div key={t.id}
                ...
                <button onClick={() => !manageMode && !editingId && setSelected(t.id)} className="w-full text-left p-4 rounded-xl transition-all"
                  style={{ background: selected === t.id ? "var(--accent-light)" : "var(--bg-surface)", border: selected === t.id ? "1.5px solid var(--accent)" : "1px solid var(--planning-border-soft)", cursor: manageMode ? "grab" : "pointer" }}>
                    ...
                    <div className="flex-1 min-w-0">
                      ...
                      <div className="flex flex-wrap gap-1 mt-2">
                        {t.activiteiten.map((a, j) => (
                          <span key={j} className="px-2 py-0.5 rounded-full text-[9px]" style={{ background: "var(--app-navy)", color: "var(--text-muted)" }}>{a}</span>
                        ))}
                      </div>
                    </div>
                  </div>
        ...
        {!manageMode && !editingId && (
          confirmLoad && selectedTpl ? (
            <div className="p-3 rounded-xl" style={{ background: "var(--warn-light)", border: "1px solid var(--warn-border)" }}>
              ...
              <div className="flex gap-2">
                <button onClick={() => setConfirmLoad(false)} className="flex-1 py-1.5 rounded-lg text-xs" style={{ border: "1px solid var(--planning-border-soft)", color: "var(--text-muted)" }}>Annuleren</button>
            ...
            <div className="flex gap-2">
              <button onClick={onClose} className="flex-1 py-2 rounded-lg text-xs" style={{ border: "1px solid var(--planning-border-soft)", color: "var(--text-muted)" }}>Annuleren</button>
        ...
        {deleteConfirm && (
          <div className="p-3 rounded-xl" style={{ background: "var(--danger-light)", border: "1px solid var(--danger)" }}>
            ...
            <div className="flex gap-2">
              <button onClick={() => setDeleteConfirm(null)} className="flex-1 py-1.5 rounded-lg text-xs" style={{ border: "1px solid var(--planning-border-soft)", color: "var(--text-muted)" }}>Annuleren</button>
...
function ConfirmDialog({ title, text, confirmLabel, confirmColor, onCancel, onConfirm }: {
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center" style={{ background: "rgba(0,0,0,0.4)" }}>
      <div className="rounded-2xl p-6 w-full max-w-sm space-y-4" style={{ background: "var(--app-navy)", border: "1px solid var(--planning-border-soft)" }}>
        <h3 className="text-base font-bold" style={{ color: "var(--text-primary)" }}>{title}</h3>
        <p className="text-sm" style={{ color: "var(--text-muted)" }}>{text}</p>
        <div className="flex gap-2">
          <button onClick={onCancel} className="flex-1 py-2 rounded-lg text-xs" style={{ border: "1px solid var(--planning-border-soft)", color: "var(--text-muted)" }}>Annuleren</button>
