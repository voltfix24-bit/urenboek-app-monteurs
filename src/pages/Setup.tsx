...
export default function Setup() {
  ...
  const passwordStrength = () => {
    ...
    if (hasSpecial && hasNumbers) return { label: "Sterk", color: "var(--accent)" };
    if (hasNumbers) return { label: "Goed", color: "var(--accent)" };
    return { label: "Matig", color: "var(--warn-text)" };
  };
          ...
          {[1, 2].map(s => (
            <div key={s} className="flex items-center gap-2">
              <div className="w-8 h-8 rounded-full flex items-center justify-center text-xs font-bold" style={{
                background: step > s ? "var(--accent)" : step === s ? "var(--accent)" : "var(--bg-surface-2)",
              ...
              {s < 2 && <div className="w-8 h-px" style={{ background: "var(--planning-border-soft)" }} />}
            </div>
          ))}
        </div>
              ...
              <div className="space-y-3">
                <div className="flex gap-3">
                  <div className="flex-1 space-y-1">
                    ...
                    <input value={form.firstName} onChange={e => setForm({ ...form, firstName: e.target.value })} placeholder="Jan" className="w-full px-3 py-2.5 rounded-xl text-sm" style={{ background: "var(--app-navy)", border: "1px solid var(--planning-border-soft)", color: "var(--text-primary)" }} />
                  </div>
                  <div className="flex-1 space-y-1">
                    <label className="text-[11px] font-semibold uppercase tracking-wider" style={{ color: "var(--text-muted)" }}>Achternaam</label>
                    <input value={form.lastName} onChange={e => setForm({ ...form, lastName: e.target.value })} placeholder="Jansen" className="w-full px-3 py-2.5 rounded-xl text-sm" style={{ background: "var(--app-navy)", border: "1px solid var(--planning-border-soft)", color: "var(--text-primary)" }} />
                  </div>
                </div>
                <div className="space-y-1">
                  <label className="text-[11px] font-semibold uppercase tracking-wider" style={{ color: "var(--text-muted)" }}>E-mailadres</label>
                  <input type="email" value={form.email} onChange={e => setForm({ ...form, email: e.target.value })} placeholder="jan@terrevolt.nl" className="w-full px-3 py-2.5 rounded-xl text-sm" style={{ background: "var(--app-navy)", border: "1px solid var(--planning-border-soft)", color: "var(--text-primary)" }} />
                </div>
                <div className="space-y-1">
                  <label className="text-[11px] font-semibold uppercase tracking-wider" style={{ color: "var(--text-muted)" }}>Wachtwoord</label>
                  <input type="password" value={form.password} onChange={e => setForm({ ...form, password: e.target.value })} placeholder="Min. 8 tekens" className="w-full px-3 py-2.5 rounded-xl text-sm" style={{ background: "var(--app-navy)", border: "1px solid var(--planning-border-soft)", color: "var(--text-primary)" }} />
                  {form.password && (
                ...
                <div className="space-y-1">
                  ...
                  <input type="password" value={form.passwordConfirm} onChange={e => setForm({ ...form, passwordConfirm: e.target.value })} placeholder="Herhaal wachtwoord" className="w-full px-3 py-2.5 rounded-xl text-sm" style={{ background: "var(--app-navy)", border: "1px solid var(--planning-border-soft)", color: "var(--text-primary)" }} />
            ...
            <>
              <div className="space-y-3">
                ...
                <label className="flex items-start gap-2 pt-2 cursor-pointer">
                  ...
                  <span className="text-xs" style={{ color: "var(--text-muted)" }}>Ik ben de eerste beheerder van TerreVolt Urenregistratie</span>
                </label>
              </div>
              <div className="flex gap-2">
                <button onClick={() => setStep(1)} className="flex-1 py-3 rounded-xl text-sm font-semibold" style={{ background: "var(--app-navy)", border: "1px solid var(--planning-border-soft)", color: "var(--text-muted)" }}>
                  ← Terug
                </button>
