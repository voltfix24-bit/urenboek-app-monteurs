    ...
    <>
      ...
      <div className="lg:ml-[240px] min-h-screen" style={{ background: "var(--app-navy)" }}>
        <header className="flex items-center gap-3 px-6 lg:px-10 pt-6 pb-4">
          ...
          <button onClick={exportCsv} className="px-3 py-2 rounded-lg text-xs font-medium flex items-center gap-1.5" style={{ border: "1px solid var(--planning-border-soft)", color: "var(--text-muted)" }}>
          ...
          {loading ? (
            ...
            <Accordion type="multiple" className="space-y-2">
              {Object.entries(grouped).sort(([a], [b]) => a.localeCompare(b)).map(([groep, items]) => (
                <AccordionItem key={groep} value={groep} className="rounded-[14px] overflow-hidden" style={{ border: "1px solid var(--planning-border-soft)", background: "var(--bg-surface)" }}>
                  ...
                  <AccordionContent className="px-0 pb-0">
                    ...
                    {items.map(t => {
                      ...
                      return (
                        <div key={t.id} className="grid grid-cols-[90px_1fr_50px_100px_80px_60px] items-center px-4 py-2 text-[12px]" style={{ borderTop: "1px solid var(--planning-border-soft)" }}>
                          ...
                          {isEditing ? (
                            <div className="flex items-center gap-1">
                              ...
                              <input
                                ...
                                style={{ background: "var(--app-navy)", border: "1px solid var(--accent)", color: "var(--text-primary)" }}
                              />
                              <button onClick={() => saveTarief(t)} className="w-5 h-5 rounded flex items-center justify-center" style={{ color: "var(--accent)" }}><Check className="h-3 w-3" /></button>
