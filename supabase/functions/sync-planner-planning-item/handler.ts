// Pure handler-factory voor sync-planner-planning-item.
// Vertrouwt geen project-, monteur-, datum- of activiteitgegevens uit de browser:
// alleen external_id + bereik komen uit de input, de inhoud komt opnieuw van Planner.
...
export function createHandler(deps: Deps) {
  ...
  return async (req: Request): Promise<Response> => {
    ...
    const { data: rateOk, error: rateErr } = await supaAdmin.rpc("check_rate_limit", {
      _key: `user:${userId}`,
      _endpoint: "sync-planner-planning-item",
      _limit: 10,
      _window_seconds: 60,
    });
    ...
    const audit = async (
      uitkomst: "gesynchroniseerd" | "reeds_gesynchroniseerd" | "geweigerd" | "fout",
      datum: string,
    ...
    // Laad lokale data voor classificatie.
    ...
    if (regel.status !== "nieuw") {
      await audit("geweigerd", item.datum, null, `status_${regel.status}`);
      return json(409, { error: `Status ${regel.status} kan niet als nieuwe sync worden uitgevoerd` });
    }
    if (regel.conflict_redenen.length > 0) {
      await audit("geweigerd", item.datum, null, `conflict:${regel.conflict_redenen.join(",")}`);
      return json(409, { error: "Conflict in actuele situatie" });
    }
    ...
    // Manager-profiel ophalen voor created_by.
    ...
    if (rpcErr) {
      ...
      return json(500, { error: "Schrijf-actie mislukt" });
    }
    const uitkomst = (rpcData?.uitkomst ?? "fout") as
      | "gesynchroniseerd" | "reeds_gesynchroniseerd" | "geweigerd" | "fout";
    ...
    if (uitkomst === "geweigerd") {
      ...
      return json(409, { error: rpcData?.fout_reden ?? "Sync geweigerd" });
    }
