// Pure handler-factory voor sync-planner-planning-updates (fase 2E).
// Werkt alleen Planner-regels met status 'gewijzigd' bij. Geen create, geen delete.
// Vertrouwt geen browserdata behalve datum_vanaf, datum_tot, optionele external_ids en limit.
...
export const UPDATES_BATCH_LIMIT_MAX = 25;
...
export type UpdateUitkomst = "bijgewerkt" | "overgeslagen" | "geweigerd" | "fout";

export interface UpdateRegelResultaat {
  external_id: string;
  datum: string;
  uitkomst: UpdateUitkomst;
...
export function createHandler(deps: Deps) {
  ...
  return async (req: Request): Promise<Response> => {
    ...
    const { data: rateOk, error: rateErr } = await supaAdmin.rpc("check_rate_limit", {
      _key: `user:${userId}`,
      _endpoint: "sync-planner-planning-updates",
      _limit: 5,
      _window_seconds: 60,
    });
    ...
    let limit = UPDATES_BATCH_LIMIT_MAX;
    if (body?.limit !== undefined) {
      const n = Number(body.limit);
      if (!Number.isFinite(n) || n <= 0 || Math.floor(n) !== n) {
        return json(400, { error: "Ongeldige limit" });
      }
      limit = Math.min(UPDATES_BATCH_LIMIT_MAX, n);
    }
    ...
    if (body?.external_ids !== undefined) {
      ...
      if (body.external_ids.length > UPDATES_BATCH_LIMIT_MAX) {
        return json(400, { error: `Maximaal ${UPDATES_BATCH_LIMIT_MAX} external_ids` });
      }
    ...
    // Veiligheids-precheck: blokkeer hele batch bij conflict of verwijderd_in_planner.
    if (classified.aantallen.conflict > 0 || classified.aantallen.verwijderd_in_planner > 0) {
      return json(409, {
        error: "Batch geweigerd: bereik bevat conflicten of verwijderd_in_planner",
        aantallen: classified.aantallen,
      });
    }

    let kandidaten: PreviewRegel[] = classified.regels.filter(r => r.status === "gewijzigd");
    if (externalIdsFilter) {
      ...
      if (missing.length > 0) {
        return json(409, { error: "Sommige external_ids hebben geen status 'gewijzigd'", missing });
      }
    ...
    if (kandidaten.length === 0) {
      return json(200, {
        success: true,
        verwerkt: 0,
        aantallen: { bijgewerkt: 0, overgeslagen: 0, geweigerd: 0, fout: 0 },
        resultaten: [] as UpdateRegelResultaat[],
        limit,
      });
    }
    ...
    const resultaten: UpdateRegelResultaat[] = [];
    const tellers = { bijgewerkt: 0, overgeslagen: 0, geweigerd: 0, fout: 0 };
    ...
    for (const regel of kandidaten) {
      if (!regel.urenapp_project_id || !regel.urenapp_profile_id) {
        ...
        resultaten.push({
          ...
          uitkomst: "geweigerd", planning_id: regel.bestaande_row?.id ?? null,
          fout_reden: "ontbrekende_koppeling",
        });
        await audit("geweigerd", regel.external_id, regel.datum, regel.bestaande_row?.id ?? null, "ontbrekende_koppeling");
        continue;
      }

      try {
        const { data: rpcData, error: rpcErr } = await supaAdmin.rpc("sync_planner_planning_update_v1", {
          ...
          _starttijd: regel.voorgesteld.starttijd + ":00",
          _eindtijd: regel.voorgesteld.eindtijd + ":00",
        ...
        console.error("update regel exception:", (e as Error).message);
      }
    }
